#!/bin/bash
# Verda RTX A6000 Startup Script
# Configures Granite Docling + DocLayout-YOLO for dataset processing

set -e

echo "=== Verda GPU Instance Setup ==="
...
pip install httpx google-cloud-storage aiofiles
...
nvidia-smi
python3 -c "import torch; print(f'PyTorch CUDA: {torch.cuda.is_available()}'); print(f'GPU: {torch.cuda.get_device_name(0) if torch.cuda.is_available() else \"None\"}')"

# Create processing directories
mkdir -p /workspace/processing/{input,output}
mkdir -p /workspace/models
mkdir -p /workspace/scripts

# Download DocLayout-YOLO model weights
echo "=== Downloading DocLayout-YOLO weights ==="
python3 -c "
from doclayout_yolo import YOLOv10
# This downloads the pretrained model
...
class UnifiedProcessor:
    ...
    def setup(self):
        ...
        if not self.config.skip_layout:
            ...
            self.layout_model = YOLOv10.from_pretrained("juliozhao/DocLayout-YOLO-DocStructBench")
            if torch.cuda.is_available():
                self.layout_model = self.layout_model.to("cuda")
            logger.info(f"DocLayout-YOLO ready (device: {'cuda' if torch.cuda.is_available() else 'cpu'})")
    ...
    def process_file(self, gcs_path: str) -> ProcessingResult:
        ...
        try:
            ...
            # OCR
            ocr_result = self.process_ocr(local_path) if not self.config.skip_ocr else {}

            # Layout
            layout_annotations = self.process_layout(local_path) if not self.config.skip_layout else []
    ...
    def save_results(self, results: list[ProcessingResult], batch_num: int):
        ...
        try:
            for local_file in [ocr_path, layout_path]:
                gcs_dest = f"{GCS_PREFIX}/extracted/{self.config.dataset}/{local_file.name}"
...
def main():
    parser = argparse.ArgumentParser(description="Process datasets with Docling OCR + DocLayout-YOLO")
...
chmod +x /workspace/scripts/process_datasets.py

# Create a simple test script
cat > /workspace/scripts/test_setup.py << 'TEST_SCRIPT'
#!/usr/bin/env python3
...
chmod +x /workspace/scripts/test_setup.py
...
echo "  python /workspace/scripts/test_setup.py"
echo ""
echo "To process datasets:"
echo "  python /workspace/scripts/process_datasets.py --list"
echo "  python /workspace/scripts/process_datasets.py funsd --batch-size 200"
echo ""
echo "Remember to configure GCS credentials:"
