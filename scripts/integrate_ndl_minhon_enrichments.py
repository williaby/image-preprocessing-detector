#!/usr/bin/env python3
"""Integrate enrichment sources into ndl-minhon Layer 2 metadata.

NDL-Minhon (National Diet Library Minhon) provides 32,822 historical
Japanese kuzushiji pages with 523K line-level transcriptions from the
NDL crowdsourcing project.  Images are classical Japanese texts written
in cursive kuzushiji (kanji) script, scanned from the National Diet
Library's digital collections.

License: CC-BY-SA-4.0 (copyleft — derivative works must use same license)

Enrichments applied:
  - capture_method: "scanner_flatbed" (National Diet Library scans)
  - has_handwriting: True (all samples — classical cursive writing)
  - iso639_language: "ja" (Japanese)
  - iso15924_script: "Hani" (classical kanji/kuzushiji)
  - script_family: "cjk"
  - text_direction: "ttb" (top-to-bottom vertical Japanese)
  - domain_level1: "HIS" (historical Japanese classical texts)
  - text_scope: "page"
...
Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \\
        uv run python3 scripts/integrate_ndl_minhon_enrichments.py --dry-run
"""
...
__l4_parser__ = (
    "src/image_preprocessing_detector/annotation/parsers/handwriting/ndl_minhon.py"
)
...
# ===================================================================
DATASET_NAME = "ndl-minhon"

REGISTRY_DIR = Path("/mnt/e/image_detection/metadata_registry")
METADATA_PATH = REGISTRY_DIR / "json" / "ndl_minhon_metadata.json"
...
def integrate_sample(sample: dict[str, Any]) -> dict[str, Any]:
    """Create integrated enrichment data for a single NDL-Minhon sample.
    ...
    # -------------------------------------------------------------------
    # CAPTURE METHOD
    # National Diet Library digitisation — professional flatbed scanning
    # of classical Japanese texts from the library's collections
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # DOMAIN
    # Historical Japanese classical texts (literature, religious texts,
    # government records, etc. from pre-modern Japan)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # LANGUAGE / SCRIPT
    # Japanese (ISO 639-1: ja) in Han/Kanji script (ISO 15924: Hani)
    # Classical kuzushiji — cursive variant of kanji
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # LAYOUT DETECTIONS  (not applicable — full page scans)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # CONTENT FLAGS
    # All samples are page-level classical Japanese handwriting
    # (kuzushiji cursive script)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # TEXT CONTENT
    # Page-level images with rich line-level transcription data from
    # NDL crowdsourcing (523K line annotations across 32K pages)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # IMAGE PROPERTIES
    # Derive from original_file.channels (library scans)
    # -------------------------------------------------------------------
