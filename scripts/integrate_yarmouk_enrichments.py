#!/usr/bin/env python3
"""Integrate all enrichment sources into yarmouk Layer 2 metadata.

yarmouk specifics:
  - 15,062 Arabic handwritten document images (Yarmouk OCR dataset)
  - Scanner-captured handwritten Arabic documents
  - Single script: Arabic (Arab), single language: Arabic (ar)
  - Language enrichment + Docling layout (106 batches) + OCR (106 batches)
...
DATASET_NAME = "yarmouk"
IS_SYNTHETIC_DATASET = False
REGISTRY_DIR = Path("/mnt/e/image_detection/metadata_registry")
METADATA_PATH = REGISTRY_DIR / "json" / "yarmouk_ocr_metadata.json"
LANGUAGE_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "yarmouk_ocr_language_enrichment.json"
DOCLING_LAYOUT_DIR = REGISTRY_DIR / "extracted" / "yarmouk"
DOCLING_OCR_DIR = REGISTRY_DIR / "extracted" / "yarmouk"
...
# Scanner-captured handwritten documents
...
def integrate_sample(s: dict[str, Any], la: dict[str, list[dict[str, Any]]],
                     oc: dict[str, dict[str, Any]], li: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    raw_split = s.get("source", {}).get("split", "")
    ...
    data["has_handwriting"] = True  # Handwritten Arabic dataset
...
def run_integration(md: dict[str, Any], la: dict[str, list[dict[str, Any]]],
                    oc: dict[str, dict[str, Any]], li: dict[str, dict[str, Any]],
                    dry_run: bool = False) -> dict[str, Any]:
    """Run integration."""
    stats: dict[str, Any] = {"total": 0, "integrated": 0, "layout_matched": 0, "ocr_matched": 0,
                              "has_text_content_count": 0, "split_dist": Counter()}
    ...
    for sample in md["samples"]:
        ...
        if not dry_run:
            nv = {"version": ENRICHMENT_VERSION_NUMBER, "created_at": now,
                  ...
                  "description": f"Integrated enrichment {ENRICHMENT_VERSION_TAG}: Docling + docs",
...
def main() -> int:
    ...
    log.info("Done in %.2fs. %d integrated, %d layout, %d OCR, %d text",
             time.monotonic() - t0, stats["integrated"], stats["layout_matched"],
             stats["ocr_matched"], stats["has_text_content_count"])
    if args.dry_run:
        log.info("Dry run")
    else:
