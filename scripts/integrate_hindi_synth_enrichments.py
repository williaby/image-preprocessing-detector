...
LANGUAGE_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "hindi_ocr_synthetic_language_enrichment.json"
...
def load_language_enrichment(path: Path) -> dict[str, dict[str, Any]]:
    ...
    return {rec["image_id"]: rec for rec in raw.get("samples", []) if rec.get("image_id")}
        ...
        for ann in coco.get("annotations", []):
            ...
            if fn:
                index.setdefault(fn, []).append({
                    ...
                    "bbox": ann.get("bbox", []), "confidence": 1.0, "area": ann.get("area", 0.0),
                })
...
def run_integration(md: dict[str, Any], la: dict[str, list[dict[str, Any]]],
    ...
    stats: dict[str, Any] = {
        ...
        "has_text_content_count": 0, "split_dist": Counter(), "lang_dist": Counter(),
        "script_family_dist": Counter(), "has_table_count": 0, "has_handwriting_count": 0,
    }
...
def main() -> int:
    ...
    log.info("Done in %.2fs. Integrated: %d, Layout: %d, OCR: %d, Text: %d",
