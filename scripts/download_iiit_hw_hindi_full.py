...
def main() -> None:
    ...
    print(
        f"Found {len(existing)} already-extracted images — will skip these.", flush=True
    )
    ...
    print(f"\n{'=' * 60}", flush=True)
    print(
        f"Complete.  {total:,} total images  ({total_new} new, {total_skips} skipped)",
        flush=True,
    )
    print(
        f"GT TSV written: {tsv_path}  ({tsv_path.stat().st_size // 1024} KB)",
        flush=True,
    )
