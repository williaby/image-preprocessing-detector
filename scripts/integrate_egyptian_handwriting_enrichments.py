#!/usr/bin/env python3
"""Integrate enrichment sources into egyptian-handwriting Layer 2 metadata.

Egyptian Handwriting Dataset provides 11,216 word-level Arabic cursive
handwriting images from 89 writers (ages 6-73).  Images are sourced from
a Parquet-based HuggingFace distribution (CC-BY-4.0) and were originally
captured via flatbed scanner.

Enrichments applied:
  - capture_method: "scanner_flatbed" (scanned handwriting samples)
  - has_handwriting: True (all samples — word-level Arabic cursive)
...
Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \\
        uv run python3 scripts/integrate_egyptian_handwriting_enrichments.py --dry-run
"""
...
__l4_parser__ = (
    ...
    "handwriting/egyptian_handwriting.py"
)
...
# ===================================================================
DATASET_NAME = "egyptian-handwriting"

REGISTRY_DIR = Path("/mnt/e/image_detection/metadata_registry")
METADATA_PATH = REGISTRY_DIR / "json" / "egyptian-handwriting_metadata.json"
...
def integrate_sample(sample: dict[str, Any]) -> dict[str, Any]:
    ...
    # -------------------------------------------------------------------
    # SPLIT
    # Derive split from source metadata; default to "train" when absent.
    # -------------------------------------------------------------------
    data["split"] = sample.get("source", {}).get("split", "train")

    # -------------------------------------------------------------------
    # CAPTURE METHOD
    # Egyptian Handwriting images are scanned handwriting samples
    # distributed via HuggingFace Parquet (original flatbed scanner).
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # DOMAIN
    # Academic handwriting research collection (Egypt)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # CONTENT FLAGS
    # All samples are word-level Arabic cursive handwriting
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # TEXT CONTENT
    # Word-level Arabic cursive handwriting; transcription may be
    # available from the Parquet label column.
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # IMAGE PROPERTIES
    # Derive from original_file.channels; scanned Arabic handwriting
    # is typically stored as RGB even when content is achromatic.
    # -------------------------------------------------------------------
