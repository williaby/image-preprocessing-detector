...
def create_rag_pipeline_diagram(output_path: str) -> None:
    ...
    ax.text(7, 9.5, 'Foundry RAG Pipeline',
            ha='center', va='top', fontsize=22, fontweight='bold', color=colors['text'])
    ax.text(7, 9.2, 'Document Processing Flow',
    ...
    def draw_modern_box(x, y, width, height, color, status='active', label='', sublabel='', bullets=None):
        ...
        status_circle = plt.Circle((x + 0.3, y + height - 0.2), 0.08,
                                   color=status_color, zorder=3)
        ...
        # Sublabel
        if sublabel:
            ax.text(x + width/2, y + height - 0.5, sublabel,
        ...
        if bullets:
            ...
            for bullet in bullets:
                ax.text(x + width/2, bullet_y, f'• {bullet}',
    ...
    # LAYER 1 - INGEST
    draw_modern_box(
        x=1.5, y=y_positions['ingest'], width=11.0, height=1.0,
        color=colors['ingest'], status='active',
        label='Ingest', sublabel='(foundry-ingest)',
        bullets=['Document Track: PDF, Office, Images', 'Audio Track: Audio, Video']
    )

    # Arrow from Ingest down
    arrow1 = FancyArrowPatch(
        (7.0, y_positions['ingest']), (7.0, y_positions['prepare'] + 1.0),
    ...
    draw_modern_box(
        x=0.8, y=y_positions['prepare'], width=5.5, height=1.0,
        color=colors['active'], status='active',
        label='Prepare-Doc', sublabel='(foundry-prepare-doc)',
        bullets=['IQA & Corrections', 'Layout Detection', 'DQS & Routing']
    )

    draw_modern_box(
        x=7.7, y=y_positions['prepare'], width=5.5, height=1.0,
        color=colors['active'], status='active',
        label='Prepare-Audio', sublabel='(foundry-prepare-audio)',
        bullets=['FFmpeg Extraction', 'Deepgram Nova-2', 'Diarization']
    )

    # Convergent arrows from Prepare services to Unify
    arrow2 = FancyArrowPatch(
        (3.5, y_positions['prepare']), (5.5, y_positions['unify'] + 0.9),
        arrowstyle='->', mutation_scale=25, linewidth=2.5,
        color='#7F8C8D', zorder=1
    )
    ax.add_patch(arrow2)
    ...
    arrow3 = FancyArrowPatch(
        (10.5, y_positions['prepare']), (8.5, y_positions['unify'] + 0.9),
        arrowstyle='->', mutation_scale=25, linewidth=2.5,
        color='#7F8C8D', zorder=1
    )
    ax.add_patch(arrow3)
    ...
    # LAYER 3 - UNIFY
    draw_modern_box(
        x=1.5, y=y_positions['unify'], width=11.0, height=0.9,
        color=colors['not_started'], status='not_started',
        label='Unify', sublabel='(foundry-unify) - Normalized JSON (LayoutParser schema)',
        bullets=['Docling DOM: Unified schema']
    )

    # Arrow from Unify to Chunk
    arrow4 = FancyArrowPatch(
        (7.0, y_positions['unify']), (7.0, y_positions['chunk'] + 0.9),
        arrowstyle='->', mutation_scale=25, linewidth=2.5,
        color='#7F8C8D', zorder=1
    )
    ax.add_patch(arrow4)
    ...
    # LAYER 4 - CHUNK
    draw_modern_box(
        x=1.5, y=y_positions['chunk'], width=11.0, height=0.9,
        color=colors['not_started'], status='not_started',
        label='Chunk', sublabel='(foundry-chunk)',
        bullets=['Trust Scoring', 'RAG Chunking', 'Normalization']
    )

    # Arrow from Chunk to Embed
    arrow5 = FancyArrowPatch(
        (7.0, y_positions['chunk']), (7.0, y_positions['embed'] + 0.9),
        arrowstyle='->', mutation_scale=25, linewidth=2.5,
        color='#7F8C8D', zorder=1
    )
    ax.add_patch(arrow5)
    ...
    # LAYER 5 - EMBED
    draw_modern_box(
        x=1.5, y=y_positions['embed'], width=11.0, height=0.9,
        color=colors['not_started'], status='not_started',
        label='Embed', sublabel='(foundry-embed)',
        bullets=['Embedding Gen', 'Vector DB', 'Retrieval API']
    )

    # Output label at bottom
    ax.text(7.0, 0.3, '→ Collection ID', ha='center', va='center',
            fontsize=11, fontweight='bold', color='#27AE60',
            bbox={'boxstyle': 'round,pad=0.5', 'facecolor': '#E8F5E9',
