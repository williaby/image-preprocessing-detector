#!/usr/bin/env python3
"""Integrate enrichment sources into vjroda Layer 2 metadata.

VJRODa (Vertical Japanese Receipt OCR Dataset) provides 100 vertical Japanese
government PDF pages rendered at 150 DPI.  This is a small OOD evaluation set
primarily used for orientation validation.  Government/administrative documents
contain mixed vertical and horizontal text directions.

Enrichments applied:
  - capture_method: "born_digital" (PDF rendering, not scanned)
  - has_handwriting: False (typed/printed government documents)
  - iso639_language: "ja" (Japanese)
  - iso15924_script: "Jpan" (Japanese)
  - script_family: "cjk"
  - text_direction: "ttb" (primary vertical Japanese)
  - text_directions_present: ["ttb", "ltr"] (mixed vertical + horizontal)
  - domain_level1: "ADM" (Japanese government/administrative documents)
  - text_scope: "page"
...
Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \\
        uv run python3 scripts/integrate_vjroda_enrichments.py --dry-run
"""
...
__l4_parser__ = (
    "src/image_preprocessing_detector/annotation/parsers/multilingual/vjroda.py"
)
...
METADATA_PATH = REGISTRY_DIR / "json" / "vjroda_metadata.json"
...
def integrate_sample(sample: dict[str, Any]) -> dict[str, Any]:
    """Create integrated enrichment data for a single vjroda sample.
    ...
    # -------------------------------------------------------------------
    # CAPTURE METHOD
    # Born-digital PDF pages rendered at 150 DPI — not scanned
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # DOMAIN
    # Japanese government/administrative documents
    # -------------------------------------------------------------------
    ...
    data["text_directions_present"] = ["ttb", "ltr"]
    data["text_scope_detection_method"] = "dataset_documentation"

    # -------------------------------------------------------------------
    # LAYOUT DETECTIONS  (not applicable — page-level images, no
    # layout annotations provided)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # CONTENT FLAGS
    # Government documents — may contain tables but no annotations
    # to confirm; mark False without ground truth
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # TEXT CONTENT
    # Page-level printed government Japanese text
    # -------------------------------------------------------------------
