#!/usr/bin/env python3
"""Integrate all enrichment sources into mle2e Layer 2 metadata.

TEMPLATE VERSION: 1.1.0

mle2e specifics:
  - 1,816 multilingual end-to-end scene text images (4 scripts)
  - Camera/smartphone captured (scene text images)
  - Parser provides script/language ground truth via original_labels
  - No language enrichment, no LLM enrichment
  - Docling layout extraction available (10 batch files)
  - Docling OCR extraction available (10 batch files)
  - Scripts: Chinese (Hans), English (Latn), Korean (Hang), Kannada (Knda)
...
METADATA_PATH = REGISTRY_DIR / "json" / "mle2e_metadata.json"
DOCLING_LAYOUT_DIR = REGISTRY_DIR / "extracted" / "mle2e"
DOCLING_OCR_DIR = REGISTRY_DIR / "extracted" / "mle2e"
...
# mle2e is camera-captured scene text
...
# mle2e script mappings (4 scripts from parser GT)
...
SCRIPT_TO_DIRECTIONS_PRESENT: dict[str, list[str]] = {
    "Arab": ["rtl"], "Hebr": ["rtl"],
    "Jpan": ["ltr", "ttb"], "Hans": ["ltr", "ttb"], "Hant": ["ltr", "ttb"],
    "Hang": ["ltr", "ttb"],
}
...
def compute_text_statistics(text: str) -> dict[str, Any]:
    ...
    hangul = len(re.findall(r"[\uac00-\ud7af]", clean))
    kannada = len(re.findall(r"[\u0c80-\u0cff]", clean))
    latin = len(re.findall(r"[a-zA-Z]+", clean))
    if cjk > 0:
        stats["cjk_char_count"] = cjk
    if hangul > 0:
        stats["hangul_char_count"] = hangul
    if kannada > 0:
        stats["kannada_char_count"] = kannada
    if latin > 0:
...
def standardize_class_name(class_name: str) -> str:
    """Convert Docling lowercase to DocLayNet PascalCase (KI-001)."""
...
def resolve_language_script(sample: dict[str, Any]) -> tuple[str, str, float, str]:
    ...
    Priority: original_labels > v1 data > path extraction > unknown
    """
    ...
    # Best: both language and script from parser GT
...
def run_integration(
    ...
    for sample in metadata["samples"]:
        ...
        stem = Path(fn).stem
        full = Path(fn).name
        idata = integrate_sample(sample, layout_index, ocr_index)
        stats["integrated"] += 1
        ...
        if not dry_run:
            nv = {"version": ENRICHMENT_VERSION_NUMBER, "created_at": now,
                  ...
                  "description": f"Integrated enrichment {ENRICHMENT_VERSION_TAG}: parser GT + Docling layout + OCR",
...
def print_summary(stats: dict[str, Any], total: int) -> None:
    ...
    print(f"Layout: {stats['layout_matched']}, OCR: {stats['ocr_matched']}")
    print(f"Text content: {stats['has_text_content_count']}")
    for name, key in [("Language", "lang_dist"), ("Script Family", "script_family_dist"),
