#!/usr/bin/env python3
"""Benchmark student model CPU inference performance.

Measures latency metrics for ResNet-18 student model using ONNX Runtime on CPU.
Tests single-image and batch inference performance.

Target: ≤100ms (acceptable), ≤40ms (ideal) per image
"""
...
def run_benchmark() -> dict[str, Any]:
    ...
    """
    print("=" * 60)
    print("Student Model (CPU) Benchmark")
    print("=" * 60)

    # Initialize detector with explicit CPU device
    ...
    # Target validation
    print("\nTarget Validation:")
    acceptable = single_results["mean"] <= 100.0
    ideal = single_results["mean"] <= 40.0
    ...
    # Save results
    output_path = Path("docs/benchmarks/results/student_cpu_benchmark.json")
