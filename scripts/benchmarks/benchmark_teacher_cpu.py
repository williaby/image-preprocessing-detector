#!/usr/bin/env python3
"""Benchmark teacher model CPU inference performance.

Measures latency metrics for ResNet-50 teacher model using ONNX Runtime on CPU.
Teacher model is only used for escalated/high-risk cases (5-15% of pages).

Target: ≤30ms per image (on GPU), no strict CPU target
"""
...
def run_benchmark() -> dict[str, Any]:
    ...
    """
    print("=" * 60)
    print("Teacher Model (CPU) Benchmark")
    print("=" * 60)

    # Initialize detector with teacher model and explicit CPU device
    ...
    # Note: Teacher target is ≤30ms on GPU, no strict CPU target
    # We're measuring CPU performance for reference
    print("\nNote: Teacher target (≤30ms) is for GPU inference")
    print(f"CPU performance: {single_results['mean']:.2f}ms (reference only)")

    # Compile final results
    output = {
        "benchmark": "teacher_cpu",
        "device": "cpu",
        ...
        "targets": {
            "gpu_target_ms": 30.0,
            "cpu_has_no_strict_target": True,
            "note": "Teacher used for 5-15% of pages (escalation only)",
        },
    }

    # Save results
    output_path = Path("docs/benchmarks/results/teacher_cpu_benchmark.json")
