#!/usr/bin/env python3
"""Integrate all enrichment sources into multimodal-textbook Layer 2 metadata.

TEMPLATE VERSION: 1.1.0
CREATED FROM: scripts/audit/integration_script_template.py

multimodal-textbook specifics:
  - 1.1K STEM textbook pages (born-digital)
  - Contains figures, formulas, tables in educational context
  - Has 2 enrichment sources: BASE + LANG
  - capture_method: born_digital (digitally produced textbook pages)
  - domain: EDU (educational textbook content)
  - No LLM enrichment available

Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \\
        uv run python3 scripts/integrate_multimodal_textbook_enrichments.py --dry-run
"""
...
__l4_parser__ = "src/image_preprocessing_detector/annotation/parsers/document/multimodal_textbook.py"
...
# ===================================================================
DATASET_NAME = "multimodal-textbook"
...
METADATA_PATH = REGISTRY_DIR / "json" / "multimodal_textbook_metadata.json"
LLM_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "multimodal_textbook_llm_enrichment.json"
LANGUAGE_ENRICHMENT_PATH = (
    REGISTRY_DIR / "json" / "multimodal_textbook_language_enrichment.json"
)
...
# No Docling layout for multimodal-textbook
APPLY_KI_001_LAYOUT_CASING = False

# Textbooks may contain tables, formulas, and figures
...
def resolve_language(
    ...
    """Resolve best language/script for a multimodal-textbook sample.

    Priority chain:
      1. Parser GT (confidence 0.95)
      2. Language enrichment / OpenLID (confidence 0.70)
      3. LLM vision (confidence 0.65)
      4. Fallback: English (STEM textbooks predominantly English)
    ...
    # Source 4: STEM textbooks are predominantly English
...
def integrate_sample(
    ...
    # -------------------------------------------------------------------
    # LAYOUT DETECTIONS
    # No Docling layout available; keep v1 layout if present.
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # CAPTURE METHOD (born_digital from documentation)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # CONTENT FLAGS
    # STEM textbooks commonly contain figures, formulas, and tables
    # Without VLM verification, set conservatively from documentation
    # -------------------------------------------------------------------
    data["has_table"] = False
    data["has_figure"] = True  # STEM textbooks have figures/diagrams
    data["has_formula"] = True  # STEM textbooks have math formulas
    ...
    data["content_flags_confidence"] = 0.70  # Lower without VLM verification
    data["handwriting_present"] = False

    # -------------------------------------------------------------------
    # ORIENTATION (default upright for born-digital)
    # -------------------------------------------------------------------
    ...
    data["text_scope_content_type"] = v1_data.get(
        "text_scope_content_type", "textbook_page"
    )
        ...
        if not dry_run:
            new_version = {
                ...
                "description": (
                    ...
                    "language enrichment + dataset documentation"
                ),
