#!/usr/bin/env python3
"""Integrate enrichment sources into openpecha-ocr-drutsa Layer 2 metadata.

OpenPecha OCR Drutsa contains 32,364 Tibetan line-level OCR images sourced
from woodblock prints and manuscripts.  Images are extracted from HuggingFace
Parquet files with Tibetan Unicode text transcriptions.

Enrichments applied:
  - capture_method: "scanner_flatbed" (scans of woodblock prints/manuscripts)
  - has_handwriting: False (woodblock prints, not handwritten)
  - iso639_language: "bo" (Tibetan)
  - iso15924_script: "Tibt" (Tibetan)
  - script_family: "tibetan"
  - text_direction: "ltr" (Tibetan is left-to-right)
  - domain_level1: "UNK" (Tibetan religious/historical texts)
  - text_scope: "line"
  - text_scope_content_type: "printed" (woodblock printing)
  - orientation_class: 0
  - schema_version: "2.4.0"

Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \\
        uv run python3 scripts/integrate_openpecha_ocr_drutsa_enrichments.py --dry-run
"""
...
__l4_parser__ = (
    ...
    "/multilingual/openpecha_ocr_drutsa.py"
)
...
# ===================================================================
DATASET_NAME = "openpecha-ocr-drutsa"

REGISTRY_DIR = Path("/mnt/e/image_detection/metadata_registry")
METADATA_PATH = REGISTRY_DIR / "json" / "openpecha-ocr-drutsa_metadata.json"
...
def integrate_sample(sample: dict[str, Any]) -> dict[str, Any]:
    ...
    Returns:
        New enrichment data dict with language, script, and schema fields.
    """
    ...
    # -------------------------------------------------------------------
    # CAPTURE METHOD
    # Scans of woodblock prints and manuscripts — flatbed scanner
    # -------------------------------------------------------------------
    data["capture_method"] = "scanner_flatbed"
    data["capture_confidence"] = 0.85
    data["capture_detection_method"] = "dataset_documentation"

    # -------------------------------------------------------------------
    # DOMAIN
    # Tibetan religious/historical texts — no established domain code
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # LANGUAGE / SCRIPT
    # Tibetan (ISO 639-1: bo) in Tibetan script (ISO 15924: Tibt)
    # -------------------------------------------------------------------
    data["iso639_language"] = "bo"
    data["iso15924_script"] = "Tibt"
    data["language_confidence"] = 0.99
    data["script_family"] = "tibetan"
    ...
    # -------------------------------------------------------------------
    # CONTENT FLAGS
    # Line-level OCR crops from woodblock prints — no tables/figures
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # TEXT CONTENT
    # Line-level Tibetan text; transcriptions from parquet
    # -------------------------------------------------------------------
