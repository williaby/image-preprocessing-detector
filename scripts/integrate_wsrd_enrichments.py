#!/usr/bin/env python3
"""Integrate all enrichment sources into wsrd Layer 2 metadata.

TEMPLATE VERSION: 1.1.0
CREATED FROM: scripts/audit/integration_script_template.py

wsrd specifics:
  - Shadow removal dataset from NTIRE 2023+2024 challenge
  - Camera-captured document images with shadow/shadow-free pairs
  - Paired: shadow input images + shadow-free GT
  - LLM + language enrichment available
  - capture_method override: camera_smartphone (all camera-captured)

Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
        uv run python3 scripts/integrate_wsrd_enrichments.py --dry-run
"""
...
__l4_parser__ = "src/image_preprocessing_detector/annotation/parsers/correction/wsrd.py"
...
METADATA_PATH = REGISTRY_DIR / "json" / "wsrd_metadata.json"
LLM_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "wsrd_llm_enrichment.json"
LANGUAGE_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "wsrd_language_enrichment.json"

# Uncomment the enrichment sources that apply to your dataset:
# DOCLING_LAYOUT_PATH = REGISTRY_DIR / "enrichments" / "wsrd_docling_layout.json"
# DOCLING_OCR_PATH = REGISTRY_DIR / "enrichments" / "wsrd_docling_ocr.json"
# SKEW_LABELS_PATH = Path("results/wsrd_skew_labels.json")
# RESOLUTION_LABELS_PATH = Path("results/wsrd_resolution_labels.json")
# VLM_ENRICHMENT_PATH = Path("scripts/audit/results/wsrd/vlm_test_enrichments.json")
...
# VLM_TEXT_LABELS_PATH = Path("results/wsrd_text_labels.json")
...
# Not applicable: wsrd is non-synthetic camera-captured.
...
# --- VLM Contact Sheet Overrides (2026-02-13) -------------------------
# Based on visual review of 36 stratified samples across 2 contact sheets.
# WSRD is a shadow removal dataset: tabletop photos of everyday objects
# (cans, electronics, model buildings, coins) with directional shadows.
# Images contain minimal text (brand labels). NOT a document dataset.
VLM_DOMAIN_OVERRIDE: str | None = "GENERAL"
VLM_LANGUAGE_OVERRIDE: str | None = "en"
VLM_SCRIPT_OVERRIDE: str | None = "Latn"
...
def load_train_gt(path: Path) -> dict[str, dict[str, Any]]:
    ...
    wsrd: GT is the shadow-free image counterpart.
...
def compute_text_statistics(text: str) -> dict[str, Any]:
    ...
    wsrd contains primarily printed document text with shadows.
...
def resolve_language(
    ...
    # wsrd is a multilingual dataset; no single default language
...
def integrate_sample(
    ...
    # -------------------------------------------------------------------
    # DOMAIN (VLM contact sheet override -> LLM enrichment -> fallback)
    #
    # VLM contact sheet review (2026-02-13): All 36 samples are tabletop
    # scene photographs of everyday objects (cans, electronics, models,
    # coins, fabric) with directional shadow effects. NOT documents.
    # Domain: GENERAL (non-document scene images).
    # -------------------------------------------------------------------
    if VLM_DOMAIN_OVERRIDE is not None:
        data["domain_level1"] = VLM_DOMAIN_OVERRIDE
        data["domain_confidence"] = 0.90
        data["domain_detection_method"] = "vlm_contact_sheet_override"
    ...
    # -------------------------------------------------------------------
    # LANGUAGE / SCRIPT (VLM override -> priority chain)
    #
    # VLM contact sheet review: Minimal text in images (brand labels
    # like Coca-Cola, Fanta, one "AVISO" sign). Predominantly English
    # incidental text. Override to "en" / "Latn".
    # -------------------------------------------------------------------
    if VLM_LANGUAGE_OVERRIDE is not None:
        lang = VLM_LANGUAGE_OVERRIDE
        script = VLM_SCRIPT_OVERRIDE or "Latn"
        lang_conf = 0.80
        lang_method = "vlm_contact_sheet_override"
    else:
