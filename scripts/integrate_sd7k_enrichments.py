#!/usr/bin/env python3
"""Integrate all enrichment sources into sd7k Layer 2 metadata.

TEMPLATE VERSION: 1.1.0
CREATED FROM: scripts/audit/integration_script_template.py

sd7k specifics:
  - High-resolution document shadow removal dataset
  - 7,239 train input + 760 test input images
  - Known issue: train has 6,479 input vs 6,478 target (1 missing pair)
...
Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
        uv run python3 scripts/integrate_sd7k_enrichments.py --dry-run
"""
...
__l4_parser__ = "src/image_preprocessing_detector/annotation/parsers/correction/sd7k.py"
...
METADATA_PATH = REGISTRY_DIR / "json" / "sd7k_metadata.json"
LLM_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "sd7k_llm_enrichment.json"
LANGUAGE_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "sd7k_language_enrichment.json"

# Uncomment the enrichment sources that apply to your dataset:
# DOCLING_LAYOUT_PATH = REGISTRY_DIR / "enrichments" / "sd7k_docling_layout.json"
# DOCLING_OCR_PATH = REGISTRY_DIR / "enrichments" / "sd7k_docling_ocr.json"
# SKEW_LABELS_PATH = Path("results/sd7k_skew_labels.json")
# RESOLUTION_LABELS_PATH = Path("results/sd7k_resolution_labels.json")
# VLM_ENRICHMENT_PATH = Path("scripts/audit/results/sd7k/vlm_test_enrichments.json")
...
# VLM_TEXT_LABELS_PATH = Path("results/sd7k_text_labels.json")
...
def load_train_gt(path: Path) -> dict[str, dict[str, Any]]:
    ...
    sd7k: GT is the shadow-free target images.
...
def integrate_sample(
    ...
    # DOMAIN - VLM contact sheet override (2026-02-13): all sd7k images
    # are printed documents (English papers + Japanese manga) with cast shadows
    ...
    # LANGUAGE / SCRIPT - VLM contact sheet: ~60% English, ~35% Japanese
    # Set majority language (en) with moderate confidence since mixed
    ...
    if lang == "und" or lang_conf < 0.3:
        # Default to English (majority) when no better source available
