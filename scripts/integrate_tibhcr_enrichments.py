#!/usr/bin/env python3
"""Integrate all enrichment sources into tibhcr Layer 2 metadata.

TEMPLATE VERSION: 1.1.0
CREATED FROM: scripts/audit/integration_script_template.py

tibhcr specifics:
  - Tibetan Handwritten Character Recognition (141,698 images)
  - Handwritten Tibetan characters (consonants + vowels)
  - Scanned at unknown DPI
  - Docling layout: 284 batch files
  - Docling OCR: 284 batch files
  - Language enrichment: empty (0 records)
  - Monolingual Tibetan (bo) with Tibetan script (Tibt)
  - original_labels has: transcription, language_code=bo, iso15924_script_code=Tibt
  - capture_method: scanner_flatbed (scanned handwriting sheets)
  - domain_level1: EDU (educational handwriting samples)
...
__l4_dataset__       = 'tibhcr'
__l4_workstream__    = 'WS3'
__l4_parser__        = 'src/image_preprocessing_detector/annotation/parsers/multilingual/tibhcr.py'
...
METADATA_PATH = REGISTRY_DIR / "json" / "tibhcr_metadata.json"
LANGUAGE_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "tibhcr_language_enrichment.json"
DOCLING_LAYOUT_DIR = REGISTRY_DIR / "extracted" / "tibhcr"
DOCLING_OCR_DIR = REGISTRY_DIR / "extracted" / "tibhcr"
...
def integrate_sample(
    ...
    # LANGUAGE / SCRIPT (from original_labels or dataset documentation)
    original_labels = sample.get("original_labels", {})
    data["iso639_language"] = original_labels.get("language_code", "bo")
    data["iso15924_script"] = original_labels.get("iso15924_script_code", "Tibt")
    ...
    # TEXT CONTENT (from original_labels transcription or OCR)
