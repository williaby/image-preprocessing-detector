...
TRAIN_IMG_DIR = Path(
    "/mnt/e/image_detection/01_base_data/language/mlt19/TrainImages/TrainImages"
)
TEST_IMG_DIR = Path(
    "/mnt/e/image_detection/01_base_data/language/mlt19/TestImages/TestImages"
)
...
def create_contact_sheet(
    ...
    for idx, sample in enumerate(samples[: COLS * ROWS]):
        ...
        if not img_path.exists():
            ...
            draw.text(
                (x + 5, y + 5), f"MISSING: {sample['filename']}", fill="red", font=font
            )
        else:
            try:
                ...
                draw.rectangle(
                    [x, y, x + THUMB_W - 1, y + THUMB_H - 1], fill=(80, 0, 0)
                )
        ...
        label_y = y + THUMB_H
        draw.rectangle(
            [x, label_y, x + THUMB_W - 1, label_y + LABEL_H - 1], fill=(20, 20, 20)
        )
        ...
        method_short = (
            method.replace("parser_gt+llm_refined", "GT+LLM")
            .replace("parser_gt_primary", "GT-pri")
            .replace("parser_gt", "GT")
            .replace("vlm_contact_sheet", "VLM")
            .replace("llm_vision", "LLM")
            .replace("openlid_v2", "OL")
            .replace("none", "-")
        )
...
def main() -> None:
    ...
    for lang in ["fr", "de", "it"]:
        ...
        if key in by_lang_split:
            ...
            categories.append(
                (
                    f"{lang}_train_refined",
                    f"{lang.upper()} Refined ({len(pool)} samples)",
                    sampled,
                )
            )
    ...
    low_conf = [
        s
        for s in samples
        if s["language_confidence"] < 0.7 and s["iso639_language"] != "und"
    ]
    if low_conf:
        sample_size = min(SAMPLES_PER_LANG_PER_SPLIT, len(low_conf))
        sampled = random.sample(low_conf, sample_size)
        categories.append(
            ("low_conf", f"LOW CONFIDENCE (<0.7, n={len(low_conf)})", sampled)
        )
    ...
    minor_eu = [
        s
        for s in samples
        if s["iso639_language"]
        in {"es", "pt", "nl", "da", "sv", "ro", "hu", "pl", "cs", "fi", "tr"}
    ]
    if minor_eu:
        sample_size = min(SAMPLES_PER_LANG_PER_SPLIT, len(minor_eu))
        sampled = random.sample(minor_eu, sample_size)
        categories.append(
            ("minor_eu", f"MINOR EUROPEAN ({len(minor_eu)} samples)", sampled)
        )
    ...
    for sheet_id, (name, title, sheet_samples) in enumerate(categories, 1):
        ...
        placed = create_contact_sheet(
            sheet_samples, f"Sheet {sheet_id}: {title}", output_path
        )
        total_placed += placed
        print(
            f"  Sheet {sheet_id:2d}: {name:30s} -> {placed}/{len(sheet_samples)} images ({output_path.name})"
        )
