#!/usr/bin/env python3
"""Integrate all enrichment sources into docalign12k Layer 2 metadata.

TEMPLATE VERSION: 1.1.0
CREATED FROM: scripts/audit/integration_script_template.py

docalign12k specifics:
  - Synthetically distorted document alignment dataset (~30K distorted images)
  - 14 distortion groups with paired flat (rectified) GT
  - Paired: distorted_hard/ + flat/ directories
  - LLM + language enrichment available
  - capture_method override: synthetic (all synthetically distorted)

Usage:
    PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
        uv run python3 scripts/integrate_docalign12k_enrichments.py --dry-run
"""
...
__l4_dataset__       = 'docalign12k'
__l4_workstream__    = 'WS3'
__l4_parser__        = 'src/image_preprocessing_detector/annotation/parsers/correction/docalign12k.py'
...
METADATA_PATH = REGISTRY_DIR / "json" / "docalign12k_metadata.json"
LLM_ENRICHMENT_PATH = REGISTRY_DIR / "json" / "docalign12k_llm_enrichment.json"
LANGUAGE_ENRICHMENT_PATH = (
    REGISTRY_DIR / "json" / "docalign12k_language_enrichment.json"
)

# Uncomment the enrichment sources that apply to your dataset:
# DOCLING_LAYOUT_PATH = REGISTRY_DIR / "enrichments" / "docalign12k_docling_layout.json"
# DOCLING_OCR_PATH = REGISTRY_DIR / "enrichments" / "docalign12k_docling_ocr.json"
# SKEW_LABELS_PATH = Path("results/docalign12k_skew_labels.json")
# RESOLUTION_LABELS_PATH = Path("results/docalign12k_resolution_labels.json")
# VLM_ENRICHMENT_PATH = Path("scripts/audit/results/docalign12k/vlm_test_enrichments.json")
...
# VLM_TEXT_LABELS_PATH = Path("results/docalign12k_text_labels.json")
...
# Applicable: docalign12k is synthetic. LLM may hallucinate handwriting.
# List VLM-confirmed handwriting samples only:
...
# Synthetically distorted document dataset - override from documentation.
...
def load_train_gt(path: Path) -> dict[str, dict[str, Any]]:
    ...
    docalign12k: GT is the aligned document images.
...
def compute_text_statistics(text: str) -> dict[str, Any]:
    ...
    docalign12k contains primarily printed document text.
...
def resolve_language(
    ...
    # docalign12k is a multilingual dataset; no single default language
