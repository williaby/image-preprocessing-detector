...
def _resolve_generated_svg(
    puml_file: Path, output_dir: Path, expected_svg: Path
) -> bool:
    """Find and rename the generated SVG to match the .puml filename.
...
def generate_svg(puml_file: Path, jar_path: Path) -> bool:
    ...
    try:
        ...
        if _resolve_generated_svg(puml_file, output_dir, expected_svg):
...
def _handle_check() -> int:
    """Check which files need regeneration and report."""
...
def _resolve_puml_files(args: argparse.Namespace) -> list[Path] | None:
    """Resolve the list of .puml files to process.

    Returns None if there is an error (e.g., file not found).
    """
    if args.file:
    ...
    if args.all:
        return find_puml_files(DIAGRAMS_DIR)

    all_files = find_puml_files(DIAGRAMS_DIR)
    return [f for f in all_files if needs_regeneration(f)]
...
def main() -> int:
    ...
    puml_files = _resolve_puml_files(args)
    ...
    success = sum(1 for f in puml_files if generate_svg(f, PLANTUML_JAR))
    failed = len(puml_files) - success
