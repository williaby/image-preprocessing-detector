# Org Workflow Enhancement Recommendations for .github Team
...
```yaml
# Update python-compatibility.yml
on:
  workflow_call:
    inputs:
      python-versions-pr:
...
```yaml
# Add to workflow header comments
...
#   Recommended frequency: Weekly or main branch only
...
1. **Default trigger**: `workflow_call` only (not auto-triggers)
2. **Documentation**: Recommend schedule-only or manual
3. **Skip-on-draft**: Default to true
4. **Fail-fast**: Default to true (for PRs)
...
1. Implement tiered matrix in python-compatibility.yml (2 hours)
2. Create workflow templates (4 hours)
3. Create cost guide (2 hours)
