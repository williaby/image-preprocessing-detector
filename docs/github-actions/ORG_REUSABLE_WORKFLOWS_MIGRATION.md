# Migration to Org Reusable Workflows - Cost Optimization
...
```bash
# 100% failure rate, redundant with org python-compatibility.yml
git rm .github/workflows/compatibility.yml

# Fix to only run on actual releases
# Will be replaced by org python-release.yml
...
# Rename old workflows (don't delete yet)
mkdir -p .github/workflows/deprecated
mv .github/workflows/ci.yml .github/workflows/deprecated/
# ... (move others)
...
# Edit CLAUDE.md to reference org workflows
...
```yaml
# In org repo: .github/workflows/python-ci.yml
on:
  workflow_call:
    inputs:
      custom-parameter:
...
# Step 2: Optimize fuzzing (remove PR trigger)
# Edit .github/workflows/cifuzzy.yml
# Change 'on:' section to schedule + push:main only
...
# (pr-checks.yml, weekly-comprehensive.yml)

# Test with draft PR
# Verify workflows run correctly
# Mark ready when validated
