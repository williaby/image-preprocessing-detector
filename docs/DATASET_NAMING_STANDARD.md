...
| `dibco` | dibco | dibco-train, historical_degraded | ✅ | Document binarization competition |
| `diqa` | diqa-5000 | diqa_5000, diqa5000 | ✅ | Document IQA benchmark |
...
| `historical-degraded` | historical_degraded | historical_degraded | ✅ | Historical document degradation |
...
| `iqa-phase7-165k` | iqa_phase7_165k | iqa_phase7, phase7-165k | ✅ | IQA training dataset (augmented) |
| `iqa-phase7-100k` | iqa_phase7_100k | iqa_phase7_100k | ✅ | IQA training dataset (100K subset) |
...
├── historical_degraded/   ← Underscore format
├── hasyv2/                ← No separator
...
├── historical-degraded/   ← Canonical kebab-case
...
mv 01_base_data/historical_degraded 01_base_data/historical-degraded
...
ALIAS_MAP = {
    "nist_db2": "nist-sd2",
    "historical_degraded": "historical-degraded",
    "hasyv2": "hasy",
    "ohr_bench": "ohr-bench",
    # ... add all mappings
}
...
| DIBCO naming | `dibco-train`, `historical_degraded` | `dibco` | `historical_degraded` is alias |
...
DATASET_REGISTRY = {
    ...
    "dibco": {
        ...
        "aliases": ["dibco-train", "historical_degraded"],
...
legacy_names = ["nist_db2", "historical_degraded", "hasyv2", "tablebank"]

# Normalize all to canonical
canonical_names = [normalize_dataset_name(name) for name in legacy_names]
# Result: ["nist-sd2", "dibco", "hasy", "tablebank"]
```
...
**Last Updated**: 2025-01-30
**Migration Status**: 🔄 In Progress (Week 1 of 4)
