# Layer 2 Metadata Audit - midv500
...
| Property | Value |
|----------|-------|
| Dataset Name | midv500 |
| Total Samples | 15050 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/midv500 |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/midv500_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/midv500.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/midv500_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/midv500_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/midv500_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/midv500_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/midv500_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/midv500_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/midv500_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/midv500_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/midv500/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/midv500/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Primary language(s) | multi | Dataset documentation |
| Primary script(s) | multi (Latn, Cyrl) | Dataset documentation |
| Capture method | camera_smartphone | Dataset documentation |
| Expected splits | See source doc | Dataset documentation |
| Total samples | 15050 | Parser manifest |
...
- [ ] Read `docs/datasets/source/midv500.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/midv500_parser.py` (if exists)
...
| `iso639_language` | multi | Dataset documentation | HIGH |
| `iso15924_script` | multi (Latn, Cyrl) | Dataset documentation | HIGH |
...
**Notes**: MIDV-500 identity documents. 3,612 images. Camera-captured ID cards/passports.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset midv500
```

**Output**: `scripts/audit/results/midv500/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset midv500 \
    --output scripts/audit/results/midv500/compliance.json
```
...
**Output**: `scripts/audit/results/midv500/comparison_report.json`
...
Document all defects in `scripts/audit/results/midv500/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/midv500/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/midv500/audit_progress.json`
...
- [ ] Create `scripts/integrate_midv500_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset midv500` (KI-001)
...
# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_midv500_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset midv500 --phase6 --verbose

# Output: scripts/audit/results/midv500/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset midv500 --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/midv500/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/midv500_contact_sheets/ \
...
# scripts/audit/results/midv500/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/midv500/vlm_corrections.json`
...
- Save to `tmp_cleanup/midv500_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/midv500/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/midv500/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/midv500/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 15050), 10)` = ___ samples
...
**Output**: `results/midv500_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_midv500_enrichments.py \
    --vlm-text-labels results/midv500_text_labels.json
```
...
- [ ] Update `docs/datasets/source/midv500.md`
...
> **Purpose**: Ensure `docs/datasets/source/midv500.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/midv500_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/midv500.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/midv500.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset midv500 --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/midv500/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/midv500/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/midv500/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/midv500/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_midv500_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/midv500/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/midv500/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/midv500.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/midv500_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/midv500/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/midv500_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_midv500_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/midv500/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/midv500/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/midv500/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/midv500_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
