# Layer 2 Metadata Audit - nist-sd6
...
| Property | Value |
|----------|-------|
| Dataset Name | nist-sd6 |
| Total Samples | 5595 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/nist-sd6 |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/nist-sd6_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/nist-sd6.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/nist-sd6_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/nist-sd6_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/nist-sd6_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/nist-sd6_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/nist-sd6_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/nist-sd6_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/nist-sd6_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/nist-sd6_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/nist-sd6/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/nist-sd6/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Total samples | 5595 | Parser manifest |
...
- [ ] Read `docs/datasets/source/nist-sd6.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/nist-sd6_parser.py` (if exists)
...
**Notes**: NIST Special Database 6. Structured tax forms. 5,595 samples. Scanner captured.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset nist-sd6
```

**Output**: `scripts/audit/results/nist-sd6/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset nist-sd6 \
    --output scripts/audit/results/nist-sd6/compliance.json
```
...
**Output**: `scripts/audit/results/nist-sd6/comparison_report.json`
...
Document all defects in `scripts/audit/results/nist-sd6/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/nist-sd6/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/nist-sd6/audit_progress.json`
...
- [ ] Create `scripts/integrate_nist-sd6_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset nist-sd6` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nist-sd6_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nist-sd6_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset nist-sd6 --phase6 --verbose

# Output: scripts/audit/results/nist-sd6/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset nist-sd6 --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/nist-sd6/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/nist-sd6_contact_sheets/ \
...
# scripts/audit/results/nist-sd6/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/nist-sd6/vlm_corrections.json`
...
- Save to `tmp_cleanup/nist-sd6_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/nist-sd6/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/nist-sd6/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/nist-sd6/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 5595), 10)` = ___ samples
...
**Output**: `results/nist-sd6_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nist-sd6_enrichments.py \
    --vlm-text-labels results/nist-sd6_text_labels.json
```
...
- [ ] Update `docs/datasets/source/nist-sd6.md`
...
> **Purpose**: Ensure `docs/datasets/source/nist-sd6.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/nist-sd6_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/nist-sd6.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/nist-sd6.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset nist-sd6 --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/nist-sd6/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_nist-sd6_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/nist-sd6.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/nist-sd6_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/nist-sd6_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_nist-sd6_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/nist-sd6/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/nist-sd6_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
