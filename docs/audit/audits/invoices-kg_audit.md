# Layer 2 Metadata Audit - invoices-kg
...
| Property | Value |
|----------|-------|
| Dataset Name | invoices-kg |
| Total Samples | 1414 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/invoices-kg |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/invoices-kg_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/invoices-kg.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/invoices-kg_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/invoices-kg_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/invoices-kg_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/invoices-kg_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/invoices-kg_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/invoices-kg_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/invoices-kg_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/invoices-kg_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/invoices-kg/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/invoices-kg/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Total samples | 1414 | Parser manifest |
...
- [ ] Read `docs/datasets/source/invoices-kg.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/invoices-kg_parser.py` (if exists)
...
**Notes**: Invoice knowledge graph dataset. 1,414 invoice images. Scanned documents.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset invoices-kg
```

**Output**: `scripts/audit/results/invoices-kg/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset invoices-kg \
    --output scripts/audit/results/invoices-kg/compliance.json
```
...
**Output**: `scripts/audit/results/invoices-kg/comparison_report.json`
...
Document all defects in `scripts/audit/results/invoices-kg/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/invoices-kg/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/invoices-kg/audit_progress.json`
...
- [ ] Create `scripts/integrate_invoices-kg_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset invoices-kg` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_invoices-kg_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_invoices-kg_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset invoices-kg --phase6 --verbose

# Output: scripts/audit/results/invoices-kg/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset invoices-kg --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/invoices-kg/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/invoices-kg_contact_sheets/ \
...
# scripts/audit/results/invoices-kg/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/invoices-kg/vlm_corrections.json`
...
- Save to `tmp_cleanup/invoices-kg_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/invoices-kg/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/invoices-kg/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/invoices-kg/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 1414), 10)` = ___ samples
...
**Output**: `results/invoices-kg_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_invoices-kg_enrichments.py \
    --vlm-text-labels results/invoices-kg_text_labels.json
```
...
- [ ] Update `docs/datasets/source/invoices-kg.md`
...
> **Purpose**: Ensure `docs/datasets/source/invoices-kg.md` is the single source of truth
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset invoices-kg --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/invoices-kg/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_invoices-kg_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/invoices-kg.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/invoices-kg_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/invoices-kg_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_invoices-kg_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/invoices-kg/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/invoices-kg_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
