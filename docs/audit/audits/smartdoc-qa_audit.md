# Layer 2 Metadata Audit - smartdoc-qa
...
| Property | Value |
|----------|-------|
| Dataset Name | smartdoc-qa |
| Total Samples | 4260 |
| Image Base Path | /mnt/e/image_detection/02_benchmark_only/smartdoc-qa |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/smartdoc-qa_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/smartdoc-qa.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/smartdoc-qa_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/smartdoc-qa_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/smartdoc-qa_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/smartdoc-qa_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/smartdoc-qa_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/smartdoc-qa_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/smartdoc-qa_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/smartdoc-qa_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/smartdoc-qa/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/smartdoc-qa/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/smartdoc-qa.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/smartdoc-qa_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset smartdoc-qa
```

**Output**: `scripts/audit/results/smartdoc-qa/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset smartdoc-qa \
    --output scripts/audit/results/smartdoc-qa/compliance.json
```
...
**Output**: `scripts/audit/results/smartdoc-qa/comparison_report.json`
...
Document all defects in `scripts/audit/results/smartdoc-qa/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/smartdoc-qa/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/smartdoc-qa/audit_progress.json`
...
- [ ] Create `scripts/integrate_smartdoc-qa_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset smartdoc-qa` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_smartdoc-qa_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_smartdoc-qa_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset smartdoc-qa --phase6 --verbose

# Output: scripts/audit/results/smartdoc-qa/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset smartdoc-qa --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/smartdoc-qa/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/smartdoc-qa_contact_sheets/ \
...
# scripts/audit/results/smartdoc-qa/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/smartdoc-qa/vlm_corrections.json`
...
- Save to `tmp_cleanup/smartdoc-qa_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/smartdoc-qa/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/smartdoc-qa/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/smartdoc-qa/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 4260), 10)` = ___ samples
...
**Output**: `results/smartdoc-qa_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_smartdoc-qa_enrichments.py \
    --vlm-text-labels results/smartdoc-qa_text_labels.json
```
...
- [ ] Update `docs/datasets/source/smartdoc-qa.md`
...
> **Purpose**: Ensure `docs/datasets/source/smartdoc-qa.md` is the single source of truth
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset smartdoc-qa --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/smartdoc-qa/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_smartdoc-qa_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/smartdoc-qa.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/smartdoc-qa_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/smartdoc-qa_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_smartdoc-qa_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/smartdoc-qa/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/smartdoc-qa_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
