# Layer 2 Metadata Audit - realdae
...
| Property | Value |
|----------|-------|
| Dataset Name | realdae |
| Total Samples | 583 |
| Image Base Path | /mnt/e/image_detection/01_base_data/camera_captured/realdae |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/realdae_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/realdae.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/realdae_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/realdae_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/realdae_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/realdae_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/realdae_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/realdae_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/realdae_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/realdae_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/realdae/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/realdae/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/realdae.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/realdae_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset realdae
```

**Output**: `scripts/audit/results/realdae/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset realdae \
    --output scripts/audit/results/realdae/compliance.json
```
...
**Output**: `scripts/audit/results/realdae/comparison_report.json`
...
Document all defects in `scripts/audit/results/realdae/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/realdae/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/realdae/audit_progress.json`
...
- [ ] Create `scripts/integrate_realdae_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset realdae` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_realdae_enrichments.py --dry-run
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset realdae --phase6 --verbose

# Output: scripts/audit/results/realdae/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset realdae --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/realdae/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/realdae_contact_sheets/ \
...
# scripts/audit/results/realdae/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/realdae/vlm_corrections.json`
...
- Save to `tmp_cleanup/realdae_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/realdae/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/realdae/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/realdae/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 583), 10)` = ___ samples
...
**Output**: `results/realdae_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_realdae_enrichments.py \
    --vlm-text-labels results/realdae_text_labels.json
```
...
- [ ] Update `docs/datasets/source/realdae.md`
...
> **Purpose**: Ensure `docs/datasets/source/realdae.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/realdae_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/realdae.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/realdae.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset realdae --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/realdae/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/realdae/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/realdae/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/realdae/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_realdae_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/realdae/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/realdae/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/realdae.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/realdae_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/realdae/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/realdae_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_realdae_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/realdae/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/realdae/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/realdae/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/realdae_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
