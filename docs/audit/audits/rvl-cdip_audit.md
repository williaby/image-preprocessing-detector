# Layer 2 Metadata Audit - rvl-cdip
...
| Total Samples | 16000 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/rvl-cdip |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/rvl-cdip_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/rvl-cdip.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/rvl-cdip_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/rvl-cdip_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/rvl-cdip_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/rvl-cdip_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/rvl-cdip_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/rvl-cdip_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/rvl-cdip_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/rvl-cdip_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/rvl-cdip/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/rvl-cdip/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Total samples | 16000 | Parser manifest |
...
- [ ] Read `docs/datasets/source/rvl-cdip.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/rvl-cdip_parser.py` (if exists)
...
**Notes**: RVL-CDIP document classification. 16,000 images (subset). 16 document classes. Scanned.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset rvl-cdip
```

**Output**: `scripts/audit/results/rvl-cdip/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset rvl-cdip \
    --output scripts/audit/results/rvl-cdip/compliance.json
```
...
**Output**: `scripts/audit/results/rvl-cdip/comparison_report.json`
...
Document all defects in `scripts/audit/results/rvl-cdip/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/rvl-cdip/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/rvl-cdip/audit_progress.json`
...
- [ ] Create `scripts/integrate_rvl-cdip_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset rvl-cdip` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_rvl-cdip_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_rvl-cdip_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset rvl-cdip --phase6 --verbose

# Output: scripts/audit/results/rvl-cdip/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset rvl-cdip --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/rvl-cdip/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/rvl-cdip_contact_sheets/ \
...
# scripts/audit/results/rvl-cdip/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/rvl-cdip/vlm_corrections.json`
...
- Save to `tmp_cleanup/rvl-cdip_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/rvl-cdip/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/rvl-cdip/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/rvl-cdip/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 16000), 10)` = ___ samples
...
**Output**: `results/rvl-cdip_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_rvl-cdip_enrichments.py \
    --vlm-text-labels results/rvl-cdip_text_labels.json
```
...
- [ ] Update `docs/datasets/source/rvl-cdip.md`
...
> **Purpose**: Ensure `docs/datasets/source/rvl-cdip.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/rvl-cdip_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/rvl-cdip.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/rvl-cdip.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset rvl-cdip --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/rvl-cdip/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_rvl-cdip_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/rvl-cdip.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/rvl-cdip_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/rvl-cdip_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_rvl-cdip_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/rvl-cdip/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/rvl-cdip_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
