...
| Dataset Name | mle2e |
| Total Samples | 1816 |
| Image Base Path | /mnt/e/image_detection/01_base_data/language/mle2e |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/mle2e_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/mle2e.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/mle2e_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/mle2e_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/mle2e_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/mle2e_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/mle2e_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/mle2e_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/mle2e_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/mle2e_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/mle2e/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/mle2e/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Primary language(s) | multi (ar, bn, de, en, fr, hi, it, ja, ko, zh) | Dataset documentation |
| Primary script(s) | multi (Latn, Arab, Deva, Beng, Jpan, Kore, Hans) | Dataset documentation |
| Capture method | camera_smartphone | Dataset documentation |
| Expected splits | See source doc | Dataset documentation |
| Total samples | 1816 | Parser manifest |
...
- [ ] Read `docs/datasets/source/mle2e.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/mle2e_parser.py` (if exists)
...
| `iso639_language` | multi (ar, bn, de, en, fr, hi, it, ja, ko, zh) | Dataset documentation | HIGH |
| `iso15924_script` | multi (Latn, Arab, Deva, Beng, Jpan, Kore, Hans) | Dataset documentation | HIGH |
...
**Notes**: MLE2E scene text detection. 1,816 natural scene images. Multilingual end-to-end.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset mle2e
```

**Output**: `scripts/audit/results/mle2e/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset mle2e \
    --output scripts/audit/results/mle2e/compliance.json
```
...
**Output**: `scripts/audit/results/mle2e/comparison_report.json`
...
Document all defects in `scripts/audit/results/mle2e/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/mle2e/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/mle2e/audit_progress.json`
...
- [ ] Create `scripts/integrate_mle2e_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset mle2e` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_mle2e_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_mle2e_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset mle2e --phase6 --verbose

# Output: scripts/audit/results/mle2e/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset mle2e --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/mle2e/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/mle2e_contact_sheets/ \
...
# scripts/audit/results/mle2e/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/mle2e/vlm_corrections.json`
...
- Save to `tmp_cleanup/mle2e_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/mle2e/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/mle2e/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/mle2e/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 1816), 10)` = ___ samples
...
**Output**: `results/mle2e_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_mle2e_enrichments.py \
    --vlm-text-labels results/mle2e_text_labels.json
```
...
- [ ] Update `docs/datasets/source/mle2e.md`
...
> **Purpose**: Ensure `docs/datasets/source/mle2e.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/mle2e_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/mle2e.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/mle2e.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset mle2e --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/mle2e/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/mle2e/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/mle2e/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/mle2e/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_mle2e_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/mle2e/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/mle2e/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/mle2e.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/mle2e_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/mle2e/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/mle2e_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_mle2e_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/mle2e/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/mle2e/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/mle2e/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/mle2e_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
