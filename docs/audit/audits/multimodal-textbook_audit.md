# Layer 2 Metadata Audit - multimodal-textbook
...
| Property | Value |
|----------|-------|
| Dataset Name | multimodal-textbook |
| Total Samples | 1113 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/multimodal-textbook |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/multimodal-textbook_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/multimodal-textbook.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/multimodal-textbook_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/multimodal-textbook_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/multimodal-textbook_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/multimodal-textbook_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/multimodal-textbook_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/multimodal-textbook_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/multimodal-textbook_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/multimodal-textbook_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/multimodal-textbook/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/multimodal-textbook/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Primary language(s) | en | Dataset documentation |
| Primary script(s) | Latn | Dataset documentation |
| Capture method | born_digital | Dataset documentation |
| Expected splits | See source doc | Dataset documentation |
| Total samples | 1113 | Parser manifest |
...
- [ ] Read `docs/datasets/source/multimodal-textbook.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/multimodal-textbook_parser.py` (if exists)
...
**Notes**: Multimodal textbook pages. 1,113 images. Educational content with figures/formulas.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset multimodal-textbook
```

**Output**: `scripts/audit/results/multimodal-textbook/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset multimodal-textbook \
    --output scripts/audit/results/multimodal-textbook/compliance.json
```
...
**Output**: `scripts/audit/results/multimodal-textbook/comparison_report.json`
...
Document all defects in `scripts/audit/results/multimodal-textbook/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/multimodal-textbook/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/multimodal-textbook/audit_progress.json`
...
- [ ] Create `scripts/integrate_multimodal-textbook_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset multimodal-textbook` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_multimodal-textbook_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_multimodal-textbook_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset multimodal-textbook --phase6 --verbose

# Output: scripts/audit/results/multimodal-textbook/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset multimodal-textbook --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/multimodal-textbook/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/multimodal-textbook_contact_sheets/ \
...
# scripts/audit/results/multimodal-textbook/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/multimodal-textbook/vlm_corrections.json`
...
- Save to `tmp_cleanup/multimodal-textbook_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/multimodal-textbook/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/multimodal-textbook/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/multimodal-textbook/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 1113), 10)` = ___ samples
...
**Output**: `results/multimodal-textbook_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_multimodal-textbook_enrichments.py \
    --vlm-text-labels results/multimodal-textbook_text_labels.json
```
...
- [ ] Update `docs/datasets/source/multimodal-textbook.md`
...
> **Purpose**: Ensure `docs/datasets/source/multimodal-textbook.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/multimodal-textbook_stats.json`

### Step 2: Materialize Reliability Summary

```bash
uv run python3 scripts/materialize_reliability_summary.py \
    --datasets multimodal-textbook \
    --update-docs \
    --force
```

- [ ] Script completed successfully
- [ ] `docs/datasets/source/multimodal-textbook.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/multimodal-textbook.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset multimodal-textbook --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/multimodal-textbook/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_multimodal-textbook_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/multimodal-textbook.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/multimodal-textbook_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/multimodal-textbook_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_multimodal-textbook_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/multimodal-textbook/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/multimodal-textbook_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
