...
| Dataset Name | cvsi |
| Total Samples | 10715 |
| Image Base Path | /mnt/e/image_detection/01_base_data/language/cvsi |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/cvsi_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/cvsi.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/cvsi_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/cvsi_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/cvsi_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/cvsi_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/cvsi_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/cvsi_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/cvsi_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/cvsi_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/cvsi/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/cvsi/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Primary language(s) | multi (Hindi, Kannada, Telugu, Tamil, Bengali, Gujarati, Marathi, Oriya, Punjabi, Urdu, English, Arabic, Chinese) | Dataset documentation |
| Primary script(s) | multi (Deva, Knda, Telu, Taml, Beng, Gujr, Arab, Hans, Latn) | Dataset documentation |
| Capture method | camera_smartphone | Dataset documentation |
| Expected splits | See source doc | Dataset documentation |
| Total samples | 10715 | Parser manifest |
...
- [ ] Read `docs/datasets/source/cvsi.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/cvsi_parser.py` (if exists)
...
| `iso639_language` | multi (Hindi, Kannada, Telugu, Tamil, Bengali, Gujarati, Marathi, Oriya, Punjabi, Urdu, English, Arabic, Chinese) | Dataset documentation | HIGH |
| `iso15924_script` | multi (Deva, Knda, Telu, Taml, Beng, Gujr, Arab, Hans, Latn) | Dataset documentation | HIGH |
...
| `domain_level1` | UNK | Dataset content type | HIGH |

**Notes**: CVSI-15 video script identification. 10,715 scene text clips. 13 scripts.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset cvsi
```

**Output**: `scripts/audit/results/cvsi/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset cvsi \
    --output scripts/audit/results/cvsi/compliance.json
```
...
**Output**: `scripts/audit/results/cvsi/comparison_report.json`
...
Document all defects in `scripts/audit/results/cvsi/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/cvsi/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/cvsi/audit_progress.json`
...
- [ ] Create `scripts/integrate_cvsi_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset cvsi` (KI-001)
...
# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_cvsi_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset cvsi --phase6 --verbose

# Output: scripts/audit/results/cvsi/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset cvsi --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/cvsi/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/cvsi_contact_sheets/ \
...
# scripts/audit/results/cvsi/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/cvsi/vlm_corrections.json`
...
- Save to `tmp_cleanup/cvsi_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/cvsi/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/cvsi/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/cvsi/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 10715), 10)` = ___ samples
...
**Output**: `results/cvsi_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_cvsi_enrichments.py \
    --vlm-text-labels results/cvsi_text_labels.json
```
...
- [ ] Update `docs/datasets/source/cvsi.md`
...
> **Purpose**: Ensure `docs/datasets/source/cvsi.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/cvsi_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/cvsi.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/cvsi.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset cvsi --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/cvsi/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/cvsi/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/cvsi/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/cvsi/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_cvsi_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/cvsi/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/cvsi/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/cvsi.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/cvsi_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/cvsi/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/cvsi_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_cvsi_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/cvsi/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/cvsi/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/cvsi/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/cvsi_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
