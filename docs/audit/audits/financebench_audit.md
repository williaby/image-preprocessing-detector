# Layer 2 Metadata Audit - financebench
...
| Property | Value |
|----------|-------|
| Dataset Name | financebench |
| Total Samples | 54120 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/financebench |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/financebench_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/financebench.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/financebench_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/financebench_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/financebench_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/financebench_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/financebench_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/financebench_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/financebench_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/financebench_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/financebench/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/financebench/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Total samples | 54120 | Parser manifest |
...
- [ ] Read `docs/datasets/source/financebench.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/financebench_parser.py` (if exists)
...
**Notes**: Financial document pages from SEC filings. 54,120 pages. Born-digital PDFs.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset financebench
```

**Output**: `scripts/audit/results/financebench/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --output scripts/audit/results/financebench/compliance.json
```
...
**Output**: `scripts/audit/results/financebench/comparison_report.json`
...
Document all defects in `scripts/audit/results/financebench/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/financebench/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/financebench/audit_progress.json`
...
- [ ] Create `scripts/integrate_financebench_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset financebench` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_financebench_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_financebench_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset financebench --phase6 --verbose

# Output: scripts/audit/results/financebench/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset financebench --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/financebench/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/financebench_contact_sheets/ \
...
# scripts/audit/results/financebench/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/financebench/vlm_corrections.json`
...
- Save to `tmp_cleanup/financebench_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/financebench/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/financebench/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/financebench/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 54120), 10)` = ___ samples
...
**Output**: `results/financebench_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_financebench_enrichments.py \
    --vlm-text-labels results/financebench_text_labels.json
```
...
- [ ] Update `docs/datasets/source/financebench.md`
...
> **Purpose**: Ensure `docs/datasets/source/financebench.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/financebench_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/financebench.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/financebench.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset financebench --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/financebench/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/financebench/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/financebench/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/financebench/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_financebench_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/financebench/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/financebench/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/financebench.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/financebench_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/financebench/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/financebench_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_financebench_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/financebench/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/financebench/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/financebench/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/financebench_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
