# Layer 2 Metadata Audit - im2latex
...
| Property | Value |
|----------|-------|
| Dataset Name | im2latex |
| Total Samples | 10000 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/im2latex |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/im2latex_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/im2latex.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/im2latex_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/im2latex_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/im2latex_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/im2latex_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/im2latex_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/im2latex_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/im2latex_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/im2latex_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/im2latex/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/im2latex/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/im2latex.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/im2latex_parser.py` (if exists)
...
|-------|---------------|--------|------------|
| `capture_method` | synthetic | Dataset documentation | HIGH |
...
| `is_synthetic` | true | Dataset characteristics | HIGH |
| `domain_level1` | SCI | Dataset content type | HIGH |

**Notes**: Image-to-LaTeX formula recognition. 10,000 formula images. Synthetically rendered.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset im2latex
```

**Output**: `scripts/audit/results/im2latex/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset im2latex \
    --output scripts/audit/results/im2latex/compliance.json
```
...
**Output**: `scripts/audit/results/im2latex/comparison_report.json`
...
Document all defects in `scripts/audit/results/im2latex/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/im2latex/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/im2latex/audit_progress.json`
...
- [ ] Create `scripts/integrate_im2latex_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset im2latex` (KI-001)
...
# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_im2latex_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset im2latex --phase6 --verbose

# Output: scripts/audit/results/im2latex/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset im2latex --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/im2latex/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/im2latex_contact_sheets/ \
...
# scripts/audit/results/im2latex/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/im2latex/vlm_corrections.json`
...
- Save to `tmp_cleanup/im2latex_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/im2latex/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/im2latex/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/im2latex/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 10000), 10)` = ___ samples
...
**Output**: `results/im2latex_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_im2latex_enrichments.py \
    --vlm-text-labels results/im2latex_text_labels.json
```
...
- [ ] Update `docs/datasets/source/im2latex.md`
...
> **Purpose**: Ensure `docs/datasets/source/im2latex.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/im2latex_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/im2latex.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/im2latex.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset im2latex --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/im2latex/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/im2latex/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/im2latex/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/im2latex/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_im2latex_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/im2latex/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/im2latex/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/im2latex.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/im2latex_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/im2latex/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/im2latex_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_im2latex_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/im2latex/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/im2latex/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/im2latex/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/im2latex_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
