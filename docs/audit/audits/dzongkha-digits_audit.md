# Layer 2 Metadata Audit - dzongkha-digits
...
| Property | Value |
|----------|-------|
| Dataset Name | dzongkha-digits |
| Total Samples | 62 |
| Image Base Path | /mnt/e/image_detection/01_base_data/language/multilingual_scripts/dzongkha_digits |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/dzongkha-digits_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/dzongkha-digits.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/dzongkha-digits_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/dzongkha-digits_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/dzongkha-digits_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/dzongkha-digits_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/dzongkha-digits_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/dzongkha-digits_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/dzongkha-digits_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/dzongkha-digits_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/dzongkha-digits/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/dzongkha-digits/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/dzongkha-digits.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/dzongkha-digits_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset dzongkha-digits
```

**Output**: `scripts/audit/results/dzongkha-digits/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset dzongkha-digits \
    --output scripts/audit/results/dzongkha-digits/compliance.json
```
...
**Output**: `scripts/audit/results/dzongkha-digits/comparison_report.json`
...
Document all defects in `scripts/audit/results/dzongkha-digits/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/dzongkha-digits/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/dzongkha-digits/audit_progress.json`
...
- [ ] Create `scripts/integrate_dzongkha-digits_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset dzongkha-digits` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_dzongkha-digits_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_dzongkha-digits_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset dzongkha-digits --phase6 --verbose

# Output: scripts/audit/results/dzongkha-digits/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset dzongkha-digits --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/dzongkha-digits/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/dzongkha-digits_contact_sheets/ \
...
# scripts/audit/results/dzongkha-digits/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/dzongkha-digits/vlm_corrections.json`
...
- Save to `tmp_cleanup/dzongkha-digits_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/dzongkha-digits/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/dzongkha-digits/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/dzongkha-digits/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 62), 10)` = ___ samples
...
**Output**: `results/dzongkha-digits_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_dzongkha-digits_enrichments.py \
    --vlm-text-labels results/dzongkha-digits_text_labels.json
```
...
- [ ] Update `docs/datasets/source/dzongkha-digits.md`
...
> **Purpose**: Ensure `docs/datasets/source/dzongkha-digits.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/dzongkha-digits_stats.json`

### Step 2: Materialize Reliability Summary

```bash
uv run python3 scripts/materialize_reliability_summary.py \
    --datasets dzongkha-digits \
    --update-docs \
    --force
```

- [ ] Script completed successfully
- [ ] `docs/datasets/source/dzongkha-digits.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/dzongkha-digits.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset dzongkha-digits --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/dzongkha-digits/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_dzongkha-digits_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/dzongkha-digits.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/dzongkha-digits_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/dzongkha-digits_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_dzongkha-digits_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/dzongkha-digits/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/dzongkha-digits_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
