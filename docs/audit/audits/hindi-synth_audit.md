# Layer 2 Metadata Audit - hindi-synth
...
| Property | Value |
|----------|-------|
| Dataset Name | hindi-synth |
| Total Samples | 80008 |
| Image Base Path | /mnt/e/image_detection/01_base_data/language/hindi_ocr_synthetic |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/hindi-synth_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/hindi-synth.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/hindi-synth_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/hindi-synth_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/hindi-synth_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/hindi-synth_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/hindi-synth_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/hindi-synth_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/hindi-synth_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/hindi-synth_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/hindi-synth/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/hindi-synth/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/hindi-synth.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/hindi-synth_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset hindi-synth
```

**Output**: `scripts/audit/results/hindi-synth/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset hindi-synth \
    --output scripts/audit/results/hindi-synth/compliance.json
```
...
**Output**: `scripts/audit/results/hindi-synth/comparison_report.json`
...
Document all defects in `scripts/audit/results/hindi-synth/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/hindi-synth/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/hindi-synth/audit_progress.json`
...
- [ ] Create `scripts/integrate_hindi-synth_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset hindi-synth` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_hindi-synth_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_hindi-synth_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset hindi-synth --phase6 --verbose

# Output: scripts/audit/results/hindi-synth/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset hindi-synth --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/hindi-synth/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/hindi-synth_contact_sheets/ \
...
# scripts/audit/results/hindi-synth/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/hindi-synth/vlm_corrections.json`
...
- Save to `tmp_cleanup/hindi-synth_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/hindi-synth/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/hindi-synth/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/hindi-synth/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 80008), 10)` = ___ samples
...
**Output**: `results/hindi-synth_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_hindi-synth_enrichments.py \
    --vlm-text-labels results/hindi-synth_text_labels.json
```
...
- [ ] Update `docs/datasets/source/hindi-synth.md`
...
> **Purpose**: Ensure `docs/datasets/source/hindi-synth.md` is the single source of truth
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset hindi-synth --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/hindi-synth/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_hindi-synth_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/hindi-synth.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/hindi-synth_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/hindi-synth_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_hindi-synth_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/hindi-synth/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/hindi-synth_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
