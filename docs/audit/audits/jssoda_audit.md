...
| Dataset Name | jssoda |
| Total Samples | 2000 |
| Image Base Path | /mnt/e/image_detection/01_base_data/language/multilingual_scripts/jssoda |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/jssoda_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/jssoda.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/jssoda_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/jssoda_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/jssoda_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/jssoda_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/jssoda_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/jssoda_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/jssoda_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/jssoda_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/jssoda/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/jssoda/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/jssoda.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/jssoda_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset jssoda
```
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset jssoda \
    --output scripts/audit/results/jssoda/compliance.json
```
...
**Output**: `scripts/audit/results/jssoda/comparison_report.json`
...
Document all defects in `scripts/audit/results/jssoda/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/jssoda/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/jssoda/audit_progress.json`
...
- [ ] Run `standardize_layout_labels.py --dataset jssoda` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_jssoda_enrichments.py --dry-run
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset jssoda --phase6 --verbose

# Output: scripts/audit/results/jssoda/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset jssoda --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/jssoda/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/jssoda_contact_sheets/ \
...
# scripts/audit/results/jssoda/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/jssoda/vlm_corrections.json`
...
- Save to `tmp_cleanup/jssoda_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/jssoda/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/jssoda/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/jssoda/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 2000), 10)` = ___ samples
...
**Output**: `results/jssoda_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_jssoda_enrichments.py \
    --vlm-text-labels results/jssoda_text_labels.json
```
...
- [ ] Update `docs/datasets/source/jssoda.md`
...
> **Purpose**: Ensure `docs/datasets/source/jssoda.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/jssoda_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/jssoda.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/jssoda.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset jssoda --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/jssoda/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/jssoda/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/jssoda/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/jssoda/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_jssoda_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/jssoda/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/jssoda/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/jssoda.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/jssoda_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/jssoda/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/jssoda_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_jssoda_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/jssoda/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/jssoda/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/jssoda/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/jssoda_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
