...
|----------|-------|
| Dataset Name | tibhcr |
| Total Samples | 141698 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/tibhcr |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/tibhcr_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/tibhcr.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/tibhcr_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/tibhcr_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/tibhcr_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/tibhcr_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/tibhcr_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/tibhcr_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/tibhcr_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/tibhcr_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/tibhcr/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/tibhcr/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Primary language(s) | bo | Dataset documentation |
| Primary script(s) | Tibt | Dataset documentation |
| Capture method | scanner_flatbed | Dataset documentation |
| Expected splits | See source doc | Dataset documentation |
| Total samples | 141698 | Parser manifest |
...
- [ ] Read `docs/datasets/source/tibhcr.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/tibhcr_parser.py` (if exists)
...
| `iso639_language` | bo | Dataset documentation | HIGH |
| `iso15924_script` | Tibt | Dataset documentation | HIGH |
| `script_family` | tibetan | Derived from script | HIGH |
...
**Notes**: Tibetan handwritten character recognition. 141,698 samples. Scanned handwriting.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset tibhcr
```

**Output**: `scripts/audit/results/tibhcr/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset tibhcr \
    --output scripts/audit/results/tibhcr/compliance.json
```
...
**Output**: `scripts/audit/results/tibhcr/comparison_report.json`
...
Document all defects in `scripts/audit/results/tibhcr/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/tibhcr/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/tibhcr/audit_progress.json`
...
- [ ] Create `scripts/integrate_tibhcr_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset tibhcr` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_tibhcr_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_tibhcr_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset tibhcr --phase6 --verbose

# Output: scripts/audit/results/tibhcr/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset tibhcr --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/tibhcr/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/tibhcr_contact_sheets/ \
...
# scripts/audit/results/tibhcr/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/tibhcr/vlm_corrections.json`
...
- Save to `tmp_cleanup/tibhcr_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/tibhcr/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/tibhcr/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/tibhcr/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 141698), 10)` = ___ samples
...
**Output**: `results/tibhcr_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_tibhcr_enrichments.py \
    --vlm-text-labels results/tibhcr_text_labels.json
```
...
- [ ] Update `docs/datasets/source/tibhcr.md`
...
> **Purpose**: Ensure `docs/datasets/source/tibhcr.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/tibhcr_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/tibhcr.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/tibhcr.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset tibhcr --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/tibhcr/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/tibhcr/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/tibhcr/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/tibhcr/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_tibhcr_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/tibhcr/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/tibhcr/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/tibhcr.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/tibhcr_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/tibhcr/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/tibhcr_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_tibhcr_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/tibhcr/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/tibhcr/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/tibhcr/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/tibhcr_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
