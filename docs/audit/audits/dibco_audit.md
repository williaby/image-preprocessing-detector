...
| Dataset Name | dibco |
| Total Samples | 212 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/dibco |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/dibco_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/dibco.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/dibco_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/dibco_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/dibco_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/dibco_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/dibco_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/dibco_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/dibco_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/dibco_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/dibco/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/dibco/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/dibco.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/dibco_parser.py` (if exists)
...
| `iso15924_script` | multi (Latn, Grek) | Dataset documentation | HIGH |
...
**Notes**: DIBCO binarization competition. 212 historical document images. Benchmark-only.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset dibco
```

**Output**: `scripts/audit/results/dibco/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset dibco \
    --output scripts/audit/results/dibco/compliance.json
```
...
**Output**: `scripts/audit/results/dibco/comparison_report.json`
...
Document all defects in `scripts/audit/results/dibco/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/dibco/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/dibco/audit_progress.json`
...
- [ ] Create `scripts/integrate_dibco_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset dibco` (KI-001)
...
# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_dibco_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset dibco --phase6 --verbose

# Output: scripts/audit/results/dibco/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset dibco --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/dibco/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/dibco_contact_sheets/ \
...
# scripts/audit/results/dibco/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/dibco/vlm_corrections.json`
...
- Save to `tmp_cleanup/dibco_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/dibco/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/dibco/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/dibco/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 212), 10)` = ___ samples
...
**Output**: `results/dibco_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_dibco_enrichments.py \
    --vlm-text-labels results/dibco_text_labels.json
```
...
- [ ] Update `docs/datasets/source/dibco.md`
...
> **Purpose**: Ensure `docs/datasets/source/dibco.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/dibco_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/dibco.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/dibco.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset dibco --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/dibco/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/dibco/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/dibco/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/dibco/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_dibco_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/dibco/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/dibco/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/dibco.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/dibco_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/dibco/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/dibco_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_dibco_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/dibco/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/dibco/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/dibco/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/dibco_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
