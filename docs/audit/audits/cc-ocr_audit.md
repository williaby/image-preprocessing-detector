...
| Dataset Name | cc-ocr |
| Total Samples | 6284 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/cc-ocr |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/cc-ocr_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/cc-ocr.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/cc-ocr_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/cc-ocr_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/cc-ocr_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/cc-ocr_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/cc-ocr_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/cc-ocr_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/cc-ocr_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/cc-ocr_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/cc-ocr/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/cc-ocr/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/cc-ocr.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/cc-ocr_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset cc-ocr
```

**Output**: `scripts/audit/results/cc-ocr/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset cc-ocr \
    --output scripts/audit/results/cc-ocr/compliance.json
```
...
**Output**: `scripts/audit/results/cc-ocr/comparison_report.json`
...
Document all defects in `scripts/audit/results/cc-ocr/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/cc-ocr/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/cc-ocr/audit_progress.json`
...
- [ ] Create `scripts/integrate_cc-ocr_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset cc-ocr` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_cc-ocr_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_cc-ocr_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset cc-ocr --phase6 --verbose

# Output: scripts/audit/results/cc-ocr/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset cc-ocr --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/cc-ocr/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/cc-ocr_contact_sheets/ \
...
# scripts/audit/results/cc-ocr/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/cc-ocr/vlm_corrections.json`
...
- Save to `tmp_cleanup/cc-ocr_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/cc-ocr/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/cc-ocr/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/cc-ocr/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 6284), 10)` = ___ samples
...
**Output**: `results/cc-ocr_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_cc-ocr_enrichments.py \
    --vlm-text-labels results/cc-ocr_text_labels.json
```
...
- [ ] Update `docs/datasets/source/cc-ocr.md`
...
> **Purpose**: Ensure `docs/datasets/source/cc-ocr.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/cc-ocr_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/cc-ocr.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/cc-ocr.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset cc-ocr --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/cc-ocr/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_cc-ocr_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/cc-ocr.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/cc-ocr_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/cc-ocr_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_cc-ocr_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/cc-ocr/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/cc-ocr_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
