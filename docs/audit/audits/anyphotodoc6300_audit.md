# Layer 2 Metadata Audit - anyphotodoc6300
...
| Property | Value |
|----------|-------|
| Dataset Name | anyphotodoc6300 |
| Total Samples | 6306 |
| Image Base Path | `/mnt/e/image_detection/01_base_data/correction/anyphotodoc6300` |
| Audit Started | 2026-02-13 |
| Audit Completed | 2026-02-14 |
| Enrichment Version | v3.0.0 (VLM category-level enrichment) |
| Dataset Type | Camera-captured dewarping |
...
| Integration script | ✅ | `integrate_anyphotodoc6300_enrichments.py` |
| VLM category enrichment | ✅ | Category-level content flag assignment (v3.0.0) |
...
- **Splits**: No official splits (6306 distorted images)
...
- **Script**: `scripts/integrate_anyphotodoc6300_enrichments.py`
- **Status**: ✅ Complete
...
- **Enrichment**: v3.0.0 VLM category-level enrichment applied before re-validation
- **Images inspected**: 12 (Track C stratified passing samples)
- **Passing accuracy**: 75.0% (9/12 correct at image level)
- **Flag-level accuracy**: 95.8%
- **Total corrections**: 3 (all FN, 0 FP)
  - has_figure: 2 FN (16.7% of samples) - init_1 category rate (33%) below 50% threshold
  - has_signature: 1 FN (8.3% of samples) - init_3 category rate (33%) below 50% threshold
- **Resolved by category enrichment**: 8/11 original corrections now resolved (has_formula:2, has_figure:4, has_table:2)
- **Assessment**: Good accuracy. Residual FN are inherent to category-level assignment where observed rates fall below the 50% threshold. Per-image VLM would resolve these but at much higher cost.
...
- **Catalog doc**: `docs/datasets/source/anyphotodoc6300.md` updated with audit results
...
|-----------|-------|---------------|------------------|----------|
| Field Coverage | 85.3 | 0.25 | 0.2778 | 23.69 |
...
| VLM Accuracy | 75.0 | 0.10 | 0.1111 | 8.33 |
| **TOTAL** | | | | **92.14 - Grade: A** |
...
3. **VLM accuracy improvement**: Category-level enrichment (v3.0.0) resolved 8/11 original FN corrections. Remaining 3 FN are sub-threshold edge cases.
