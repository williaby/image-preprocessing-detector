# Layer 2 Metadata Audit - tablebank
...
| Property | Value |
|----------|-------|
| Dataset Name | tablebank |
| Total Samples | 260025 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/tablebank |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/tablebank_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/tablebank.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/tablebank_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/tablebank_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/tablebank_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/tablebank_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/tablebank_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/tablebank_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/tablebank_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/tablebank_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/tablebank/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/tablebank/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/tablebank.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/tablebank_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset tablebank
```

**Output**: `scripts/audit/results/tablebank/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset tablebank \
    --output scripts/audit/results/tablebank/compliance.json
```
...
**Output**: `scripts/audit/results/tablebank/comparison_report.json`
...
Document all defects in `scripts/audit/results/tablebank/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/tablebank/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/tablebank/audit_progress.json`
...
- [ ] Create `scripts/integrate_tablebank_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset tablebank` (KI-001)
...
# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_tablebank_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset tablebank --phase6 --verbose

# Output: scripts/audit/results/tablebank/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset tablebank --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/tablebank/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/tablebank_contact_sheets/ \
...
# scripts/audit/results/tablebank/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/tablebank/vlm_corrections.json`
...
- Save to `tmp_cleanup/tablebank_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/tablebank/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/tablebank/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/tablebank/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 260025), 10)` = ___ samples
...
**Output**: `results/tablebank_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_tablebank_enrichments.py \
    --vlm-text-labels results/tablebank_text_labels.json
```
...
- [ ] Update `docs/datasets/source/tablebank.md`
...
> **Purpose**: Ensure `docs/datasets/source/tablebank.md` is the single source of truth
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/tablebank.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/tablebank.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset tablebank --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/tablebank/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/tablebank/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/tablebank/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/tablebank/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_tablebank_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/tablebank/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/tablebank/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/tablebank.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/tablebank_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/tablebank/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/tablebank_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_tablebank_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/tablebank/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/tablebank/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/tablebank/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/tablebank_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
