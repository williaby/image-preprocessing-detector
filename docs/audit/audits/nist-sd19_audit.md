# Layer 2 Metadata Audit - nist-sd19
...
| Property | Value |
|----------|-------|
| Dataset Name | nist-sd19 |
| Total Samples | 3669 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/nist-sd19 |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/nist-sd19_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/nist-sd19.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/nist-sd19_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/nist-sd19_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/nist-sd19_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/nist-sd19_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/nist-sd19_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/nist-sd19_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/nist-sd19_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/nist-sd19_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/nist-sd19/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/nist-sd19/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Total samples | 3669 | Parser manifest |
...
- [ ] Read `docs/datasets/source/nist-sd19.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/nist-sd19_parser.py` (if exists)
...
**Notes**: NIST Special Database 19. Handwritten digits+letters. 3,669 samples in L2. Scanner captured.
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset nist-sd19
```

**Output**: `scripts/audit/results/nist-sd19/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset nist-sd19 \
    --output scripts/audit/results/nist-sd19/compliance.json
```
...
**Output**: `scripts/audit/results/nist-sd19/comparison_report.json`
...
Document all defects in `scripts/audit/results/nist-sd19/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/nist-sd19/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/nist-sd19/audit_progress.json`
...
- [ ] Create `scripts/integrate_nist-sd19_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset nist-sd19` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nist-sd19_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nist-sd19_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset nist-sd19 --phase6 --verbose

# Output: scripts/audit/results/nist-sd19/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset nist-sd19 --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/nist-sd19/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/nist-sd19_contact_sheets/ \
...
# scripts/audit/results/nist-sd19/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/nist-sd19/vlm_corrections.json`
...
- Save to `tmp_cleanup/nist-sd19_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/nist-sd19/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/nist-sd19/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/nist-sd19/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 3669), 10)` = ___ samples
...
**Output**: `results/nist-sd19_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nist-sd19_enrichments.py \
    --vlm-text-labels results/nist-sd19_text_labels.json
```
...
- [ ] Update `docs/datasets/source/nist-sd19.md`
...
> **Purpose**: Ensure `docs/datasets/source/nist-sd19.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/nist-sd19_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/nist-sd19.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/nist-sd19.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset nist-sd19 --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/nist-sd19/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_nist-sd19_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/nist-sd19.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/nist-sd19_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/nist-sd19_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_nist-sd19_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/nist-sd19/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/nist-sd19_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
