# Layer 2 Metadata Audit - arabic-docs-ocr
...
| Property | Value |
|----------|-------|
| Dataset Name | arabic-docs-ocr |
| Total Samples | 8203 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/arabic-docs-ocr |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/arabic-docs-ocr_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/arabic-docs-ocr.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/arabic-docs-ocr_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/arabic-docs-ocr_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/arabic-docs-ocr_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/arabic-docs-ocr_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/arabic-docs-ocr_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/arabic-docs-ocr_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/arabic-docs-ocr_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/arabic-docs-ocr_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/arabic-docs-ocr/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/arabic-docs-ocr/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/arabic-docs-ocr.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/arabic-docs-ocr_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset arabic-docs-ocr
```

**Output**: `scripts/audit/results/arabic-docs-ocr/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset arabic-docs-ocr \
    --output scripts/audit/results/arabic-docs-ocr/compliance.json
```
...
**Output**: `scripts/audit/results/arabic-docs-ocr/comparison_report.json`
...
Document all defects in `scripts/audit/results/arabic-docs-ocr/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/arabic-docs-ocr/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/arabic-docs-ocr/audit_progress.json`
...
- [ ] Create `scripts/integrate_arabic-docs-ocr_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset arabic-docs-ocr` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_arabic-docs-ocr_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_arabic-docs-ocr_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset arabic-docs-ocr --phase6 --verbose

# Output: scripts/audit/results/arabic-docs-ocr/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset arabic-docs-ocr --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/arabic-docs-ocr/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/arabic-docs-ocr_contact_sheets/ \
...
# scripts/audit/results/arabic-docs-ocr/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/arabic-docs-ocr/vlm_corrections.json`
...
- Save to `tmp_cleanup/arabic-docs-ocr_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/arabic-docs-ocr/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/arabic-docs-ocr/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/arabic-docs-ocr/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 8203), 10)` = ___ samples
...
**Output**: `results/arabic-docs-ocr_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_arabic-docs-ocr_enrichments.py \
    --vlm-text-labels results/arabic-docs-ocr_text_labels.json
```
...
- [ ] Update `docs/datasets/source/arabic-docs-ocr.md`
...
> **Purpose**: Ensure `docs/datasets/source/arabic-docs-ocr.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/arabic-docs-ocr_stats.json`

### Step 2: Materialize Reliability Summary

```bash
uv run python3 scripts/materialize_reliability_summary.py \
    --datasets arabic-docs-ocr \
    --update-docs \
    --force
```

- [ ] Script completed successfully
- [ ] `docs/datasets/source/arabic-docs-ocr.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/arabic-docs-ocr.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset arabic-docs-ocr --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/arabic-docs-ocr/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_arabic-docs-ocr_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/arabic-docs-ocr.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/arabic-docs-ocr_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/arabic-docs-ocr_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_arabic-docs-ocr_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/arabic-docs-ocr/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/arabic-docs-ocr_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
