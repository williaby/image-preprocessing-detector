# Layer 2 Metadata Audit - signatr6k
...
| Property | Value |
|----------|-------|
| Dataset Name | signatr6k |
| Total Samples | 12514 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/signatr6k |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/signatr6k_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/signatr6k.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/signatr6k_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/signatr6k_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/signatr6k_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/signatr6k_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/signatr6k_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/signatr6k_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/signatr6k_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/signatr6k_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/signatr6k/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/signatr6k/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
| Primary language(s) | N/A | Dataset documentation |
| Primary script(s) | N/A | Dataset documentation |
| Capture method | scanner_flatbed | Dataset documentation |
| Expected splits | See source doc | Dataset documentation |
| Total samples | 12514 | Parser manifest |
...
- [ ] Read `docs/datasets/source/signatr6k.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/signatr6k_parser.py` (if exists)
...
| `iso639_language` | N/A | Dataset documentation | HIGH |
| `iso15924_script` | N/A | Dataset documentation | HIGH |
| `script_family` | N/A | Derived from script | HIGH |
| `split` | See source doc | Parser manifest | MEDIUM |
| `is_synthetic` | false | Dataset characteristics | HIGH |
| `domain_level1` | PER | Dataset content type | HIGH |

**Notes**: Signature images. 12,514 samples. No language/script (signature-only).
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset signatr6k
```

**Output**: `scripts/audit/results/signatr6k/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset signatr6k \
    --output scripts/audit/results/signatr6k/compliance.json
```
...
**Output**: `scripts/audit/results/signatr6k/comparison_report.json`
...
Document all defects in `scripts/audit/results/signatr6k/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/signatr6k/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/signatr6k/audit_progress.json`
...
- [ ] Create `scripts/integrate_signatr6k_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset signatr6k` (KI-001)
...
# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_signatr6k_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset signatr6k --phase6 --verbose

# Output: scripts/audit/results/signatr6k/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset signatr6k --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/signatr6k/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/signatr6k_contact_sheets/ \
...
# scripts/audit/results/signatr6k/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/signatr6k/vlm_corrections.json`
...
- Save to `tmp_cleanup/signatr6k_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/signatr6k/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/signatr6k/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/signatr6k/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 12514), 10)` = ___ samples
...
**Output**: `results/signatr6k_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_signatr6k_enrichments.py \
    --vlm-text-labels results/signatr6k_text_labels.json
```
...
- [ ] Update `docs/datasets/source/signatr6k.md`
...
> **Purpose**: Ensure `docs/datasets/source/signatr6k.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/signatr6k_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/signatr6k.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/signatr6k.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset signatr6k --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/signatr6k/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/signatr6k/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/signatr6k/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/signatr6k/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_signatr6k_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/signatr6k/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/signatr6k/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/signatr6k.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/signatr6k_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/signatr6k/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/signatr6k_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_signatr6k_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/signatr6k/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/signatr6k/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/signatr6k/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/signatr6k_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
