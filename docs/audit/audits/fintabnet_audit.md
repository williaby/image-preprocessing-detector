# Layer 2 Metadata Audit - fintabnet
...
| Property | Value |
|----------|-------|
| Dataset Name | fintabnet |
| Total Samples | 97475 |
| Image Base Path | /mnt/e/image_detection/01_base_datasets/fintabnet |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/fintabnet_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/fintabnet.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/fintabnet_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/fintabnet_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/fintabnet_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/fintabnet_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/fintabnet_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/fintabnet_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/fintabnet_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/fintabnet_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/fintabnet/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/fintabnet/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/fintabnet.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/fintabnet_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset fintabnet
```

**Output**: `scripts/audit/results/fintabnet/automated_screening.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset fintabnet \
    --output scripts/audit/results/fintabnet/compliance.json
```
...
**Output**: `scripts/audit/results/fintabnet/comparison_report.json`
...
Document all defects in `scripts/audit/results/fintabnet/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/fintabnet/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/fintabnet/audit_progress.json`
...
- [ ] Create `scripts/integrate_fintabnet_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset fintabnet` (KI-001)
...
# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_fintabnet_enrichments.py
```
...
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset fintabnet --phase6 --verbose

# Output: scripts/audit/results/fintabnet/phase6_track_{a,b,c}_samples.json
```
...
```bash
PYTHONPATH=. uv run python3 scripts/audit/select_audit_samples.py \
    --dataset fintabnet --phase6 --tier 3 --verbose
```
...
python scripts/audit/create_contact_sheets.py \
    --sample-json scripts/audit/results/fintabnet/phase6_track_b_samples.json \
    --output-dir tmp_cleanup/fintabnet_contact_sheets/ \
...
# scripts/audit/results/fintabnet/phase6_track_a_samples.json
```
...
**Output**: `scripts/audit/results/fintabnet/vlm_corrections.json`
...
- Save to `tmp_cleanup/fintabnet_contact_sheets/contact_sheet_NNN.jpg`
...
**Output**: `scripts/audit/results/fintabnet/vlm_test_enrichments.json`
...
```bash
# Already generated by --phase6 above; samples in:
# scripts/audit/results/fintabnet/phase6_track_c_samples.json
```
...
**Output**: `scripts/audit/results/fintabnet/vlm_validation_passing.json`
...
**Formula**: `max(ceil(0.01 * 97475), 10)` = ___ samples
...
**Output**: `results/fintabnet_text_labels.json`
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_fintabnet_enrichments.py \
    --vlm-text-labels results/fintabnet_text_labels.json
```
...
- [ ] Update `docs/datasets/source/fintabnet.md`
...
> **Purpose**: Ensure `docs/datasets/source/fintabnet.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/fintabnet_stats.json`
...
- [ ] Script completed successfully
- [ ] `docs/datasets/source/fintabnet.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/fintabnet.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset fintabnet --verbose
```
...
|------|---------|---------|----------|
| `scripts/audit/results/fintabnet/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/fintabnet/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/fintabnet/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/fintabnet/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_fintabnet_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/fintabnet/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/fintabnet/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/fintabnet.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/fintabnet_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/fintabnet/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/fintabnet_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_fintabnet_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/fintabnet/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/fintabnet/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/fintabnet/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
| `results/fintabnet_text_labels.json` | VLM text transcription labels (Phase 6.5) | [ ] | [ ] |
