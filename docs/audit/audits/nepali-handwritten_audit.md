# Layer 2 Metadata Audit - nepali-handwritten
...
| Property | Value |
|----------|-------|
| Dataset Name | nepali-handwritten |
...
- [ ] Metadata JSON exists at `/mnt/e/image_detection/metadata_registry/json/nepali-handwritten_metadata.json`?
  - **Status**:

- [ ] Dataset source doc exists at `docs/datasets/source/nepali-handwritten.md`?
  - **Status**:
...
|--------|------|---------|-------|
| Base metadata | `json/nepali-handwritten_metadata.json` | [ ] | Layer 0+1 fields |
| LLM enrichment | `enrichments/nepali-handwritten_llm_enrichment.json` | [ ] | domain, content_flags, orientation |
| Language enrichment | `enrichments/nepali-handwritten_language_enrichment.json` | [ ] | iso639, iso15924, script_family |
| Docling layout | `enrichments/nepali-handwritten_docling_layout.json` | [ ] | layout_detections with bboxes |
| Docling OCR | `enrichments/nepali-handwritten_docling_ocr.json` | [ ] | text_content, text_statistics |
| Classical IQA | `enrichments/nepali-handwritten_classical_iqa.json` | [ ] | 8 detector scores |
| Resolution quality | `results/nepali-handwritten_resolution_labels.json` | [ ] | char_height, resolution_quality_score |
| Skew/orientation | `results/nepali-handwritten_skew_labels.json` | [ ] | skew_angle, orientation_class |
| Parser/manifest | Dataset-specific | [ ] | split, source annotations |
| VLM contact sheet | `scripts/audit/results/nepali-handwritten/vlm_test_enrichments.json` | [ ] | language, script (visual batch) |
| Train GT enrichment | `scripts/audit/results/nepali-handwritten/train_gt_enrichments.json` | [ ] | language, script from GT files |
...
- [ ] Read `docs/datasets/source/nepali-handwritten.md` thoroughly
- [ ] Read `scripts/audit/results/CROSS_DATASET_KNOWN_ISSUES.json`
- [ ] Review parser source code at `src/image_preprocessing_detector/annotation/parsers/nepali-handwritten_parser.py` (if exists)
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/automated_prescreening.py --dataset nepali-handwritten
```

**Output**: `scripts/audit/results/nepali-handwritten/automated_screening.json`

### Results

13-field validation summary:
...
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    ...
    --dataset nepali-handwritten \
    --output scripts/audit/results/nepali-handwritten/compliance.json
```
...
**Output**: `scripts/audit/results/nepali-handwritten/comparison_report.json`
...
Document all defects in `scripts/audit/results/nepali-handwritten/defect_catalog.json`
...
- **Incremental save path**: `scripts/audit/results/nepali-handwritten/vlm_test_enrichments.json`
- **Progress tracking file**: `scripts/audit/results/nepali-handwritten/audit_progress.json`
...
- [ ] Create `scripts/integrate_nepali-handwritten_enrichments.py`
- [ ] Follow established integration script pattern
- [ ] Support `--dry-run` mode

### Pre-Integration Actions

- [ ] Run `standardize_layout_labels.py --dataset nepali-handwritten` (KI-001)
...
# Dry run first
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nepali-handwritten_enrichments.py --dry-run

# Actual integration
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/integrate_nepali-handwritten_enrichments.py
```
...
|-----------|-------------------|-------------------|------------------------|
| **Track A per flag** | 10 samples | 15-20 samples | 25+ samples (or all if < 50) |
| **Track C passing** | 10-15 samples | 15-25 samples | 25-40 samples |
| **Total minimum** | 15 | 30 | 60 |
| **Total target** | 40-60 | 75-120 | 120-200+ |
...
**Output**: `scripts/audit/results/nepali-handwritten/vlm_corrections.json`
...
- [ ] Generate contact sheets with Python script
  ...
  - Save to `tmp_cleanup/nepali-handwritten_contact_sheets/contact_sheet_NNN.jpg`
  - Generate manifest JSON mapping positions to filenames

**Contact sheet script**: `scripts/generate_nepali-handwritten_contact_sheets.py`
...
**Output**: `scripts/audit/results/nepali-handwritten/vlm_test_enrichments.json`
...
- [ ] Select passing samples stratified across orientations, domains, content types
  - Tier 1: 10-15 samples | Tier 2: 15-25 samples | Tier 3: 25-40 samples
- [ ] For each, read image and verify ALL populated fields
- [ ] Compute accuracy rate per field

**Output**: `scripts/audit/results/nepali-handwritten/vlm_validation_passing.json`
...
- [ ] Update `docs/datasets/source/nepali-handwritten.md`
...
> **Purpose**: Ensure `docs/datasets/source/nepali-handwritten.md` is the single source of truth
...
- [ ] Output: `metadata_registry/aggregates/nepali-handwritten_stats.json`

### Step 2: Materialize Reliability Summary

```bash
uv run python3 scripts/materialize_reliability_summary.py \
    --datasets nepali-handwritten \
    --update-docs \
    --force
```

- [ ] Script completed successfully
- [ ] `docs/datasets/source/nepali-handwritten.md` Section 12 updated
- [ ] Re-added contextual notes if needed (script overwrites entire section)

### Step 3: Update Source Doc Sections

Update `docs/datasets/source/nepali-handwritten.md` per template v1.4.0:
...
```bash
PYTHONPATH=/home/byron/dev/image_detection:$PYTHONPATH \
    uv run python3 scripts/audit/compute_scorecard.py --dataset nepali-handwritten --verbose
```
...
| Fields at 100% | 10+/13 | 8/13 | /13 | ⬜ | |
| VLM passing accuracy | 95%+ | 90% | % | ⬜ | **REQUIRED** - grade capped at D without |
| VLM images inspected (Tier 1) | 40+ | 15+ | | ⬜ | **REQUIRED** - Track A + Track C |
| VLM images inspected (Tier 2) | 75+ | 30+ | | ⬜ | Enhanced: gaps or critical defects |
| VLM images inspected (Tier 3) | 120+ | 60+ | | ⬜ | Comprehensive: major gaps or KI-009 |
...
|------|---------|---------|----------|
| `scripts/audit/results/nepali-handwritten/automated_screening.json` | Per-field pass/fail counts | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/compliance.json` | Schema validation per field | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/comparison_report.json` | Multi-source field comparison | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/defect_catalog.json` | Categorized defects with status | [ ] | [ ] |
| `scripts/integrate_nepali-handwritten_enrichments.py` | Integration script | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/vlm_corrections.json` | VLM visual inspection corrections | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/vlm_validation_passing.json` | Passing sample accuracy check | [ ] | [ ] |
| `docs/datasets/source/nepali-handwritten.md` (UPDATED) | Documentation with L2 summary + audit summary | [ ] | [ ] |
| `metadata_registry/aggregates/nepali-handwritten_stats.json` | Regenerated aggregate statistics | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/scorecard.json` | Final quality scorecard | [ ] | [ ] |

**Optional artifacts** (if applicable):

| File | Purpose | Created | Verified |
|------|---------|---------|----------|
| `tmp_cleanup/nepali-handwritten_contact_sheets/` | Contact sheet images | [ ] | [ ] |
| `scripts/generate_nepali-handwritten_contact_sheets.py` | Contact sheet generator | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/vlm_test_enrichments.json` | VLM batch classification results | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/train_gt_enrichments.json` | Train GT file extraction results | [ ] | [ ] |
| `scripts/audit/results/nepali-handwritten/audit_progress.json` | Multi-session progress tracking | [ ] | [ ] |
