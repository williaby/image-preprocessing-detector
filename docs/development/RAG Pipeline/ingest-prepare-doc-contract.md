---
schema_type: common
title: "RAG Processor → Prepare-Doc Interface Contract"
description: "Contract defining the interface between rag-processor (upstream UI)
  and Prepare-Doc (image_detection)"
tags:
- pipeline
- integration
- contract
...
- **Prepare-Doc** (Downstream): Document preprocessing, IQA assessment, corrections, and routing metadata generation
...
### 2.1 Document Types (Routed to Prepare-Doc)
...
### 2.2 Audio Types (Routed to Audio Processor - NOT Prepare-Doc)
...
RAG Processor MUST validate before routing to Prepare-Doc:
...
VALIDATION_RULES = {
    ...
    "max_pages_pdf": 500,
    "min_resolution_dpi": 72,  # Prepare-Doc will upscale if < 300 DPI
    "allowed_mime_types": [
        "application/pdf",
        "image/png",
        "image/jpeg",
...
When `output_location` is omitted from the request, Prepare-Doc uses a tenant-specific default:
...
|----------|---------------|
| **Storage Duration (TTL)** | Prepare-Doc MUST remember `request_id` for at least 72 hours |
...
Prepare-Doc sends **authenticated** status updates to the `callback_url`:
...
If callback delivery fails, Prepare-Doc MUST retry with exponential backoff:
...
```json
{
  ...
  "properties": {
    ...
    "job_id": {
      "type": "string",
      "description": "Prepare-Doc assigned job identifier"
    },
    "status": {
      "type": "string",
...
Prepare-Doc produces output conforming to the Prepare-Doc → Unify contract:
...
Prepare-Doc MUST verify that the `job_id` belongs to the tenant identified in the JWT token before processing any cancellation request. This prevents cross-tenant access vulnerabilities (Insecure Direct Object Reference).
...
```python
# Prepare-Doc URL validation rules
URL_VALIDATION = {
...
Before processing, Prepare-Doc MUST validate file content (security-critical for document upload pipelines):
...
### 9.1 Required Metrics (Prepare-Doc → RAG Processor)
...
**Trace ID propagation**: Prepare-Doc MUST include trace IDs in:
...
- [ ] Prepare-Doc unavailable → graceful degradation in RAG Processor
...
```bash
# RAG Processor configuration for Prepare-Doc
...
project-a.status              # Status updates from Prepare-Doc
...
| Prepare-Doc Team | Document processing, IQA, corrections |
