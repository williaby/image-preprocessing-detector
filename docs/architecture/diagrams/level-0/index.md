...
Prepare-Doc is the document preprocessing and quality assurance gateway. It receives raw document images from Ingest and performs comprehensive Image Quality Assessment (IQA) using a teacher-student ML architecture (ResNet-50/ResNet-18) combined with classical CV detectors for skew, blur, contrast, noise, and other degradations. Based on quality scores, it applies automatic corrections including deskewing, CLAHE enhancement, sharpening, and denoising.
