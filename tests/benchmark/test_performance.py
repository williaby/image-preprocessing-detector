...
These tests validate that key components meet performance targets from CLAUDE.md:
...
class TestClassicalIQAPerformance:
    ...
    Target: Each detector should complete in <50ms for a full-page image.
    """

    def test_blur_detection_latency(self, benchmark_image: np.ndarray) -> None:
        """Benchmark blur detection latency.

        Target: <50ms per page (300 DPI letter size)
        """
        # Warm-up run
        ...
        assert avg_time < 50, (
            f"Blur detection too slow: avg={avg_time:.1f}ms (target <50ms)"
        )
        assert max_time < 100, (
            f"Blur detection max latency too high: {max_time:.1f}ms (target <100ms)"
        )
    ...
    def test_contrast_detection_latency(self, benchmark_image: np.ndarray) -> None:
        ...
        assert avg_time < 50, (
            f"Contrast detection too slow: avg={avg_time:.1f}ms (target <50ms)"
        )

    def test_skew_detection_latency(self, benchmark_image: np.ndarray) -> None:
        """Benchmark skew detection latency.

        Target: <100ms per page (Hough transform is computationally intensive)
        """
        # Warm-up
    ...
    def test_combined_iqa_latency(self, benchmark_image: np.ndarray) -> None:
        ...
        assert avg_time < 200, (
            f"Combined IQA too slow: avg={avg_time:.1f}ms (target <200ms)"
        )
...
class TestTextGatePerformance:
    ...
    Target: <10ms per page (fast routing decision)
    """
...
class TestThroughputPerformance:
    ...
    def test_text_gate_throughput(self, benchmark_images: list[np.ndarray]) -> None:
        ...
        assert pages_per_second >= 50, (
            f"Text gate throughput too low: {pages_per_second:.1f} pages/sec (target ≥50)"
        )
...
class TestPerformanceRegression:
    ...
    These use relaxed thresholds (2x target) to catch severe regressions
    ...
    def test_no_severe_text_gate_regression(self, benchmark_image: np.ndarray) -> None:
        ...
        avg = sum(times) / len(times)
        # 2x acceptable threshold for regression detection
        assert avg < 20, f"REGRESSION: text gate avg={avg:.1f}ms (threshold 20ms)"
